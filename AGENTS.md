...
- For selectable option or choice cards, normally indicate selection with the established teal/cyan selected surface and border. A visible dot, tick, or other marker is optional and should remain only when it materially improves comprehension; do not duplicate an already obvious highlighted state. Preserve native checkbox/radio semantics, programmatic selected state, clear card-level keyboard focus, and a visual distinction that does not rely on hue alone. Do not add a coloured leading/left-edge accent rail solely to indicate selection. Reserve accent rails for non-selectable semantic messaging such as warnings, errors, or notices where appropriate.

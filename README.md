...
## Dingo's Collection Builder

A new React/Vite product, Dingo's Collection Builder, is being developed under `builder/`. The existing TMDB ID Lookup remains available at the site root, while `/builder/` provides the current development-preview welcome and collection-building interface. The Builder development preview includes a shared Blank, Decades, People, Franchises, TMDB Lists, Studios, Networks, Genres, and Streaming launcher for New Collection and New Folder. Guided families generate ordinary editable Collection → Folder → Source nodes through revalidated atomic plans while selected-folder Add Source remains a separate physical-source operation. That ordinary picker now offers Movie franchise, TMDB lists, People, Studios, Networks, Streaming, Genres, and singular Decade. TMDB Lists resolves one or more public List URLs/IDs into ordered native `LIST` sources; the owner-deployed tracked Worker route and bounded live production-path acceptance gate are complete. Decade selects one whole period or multiple individual years and always retains a general source for every selected period while optional Genre choices add separate canonical Movie, Series, or Movie-then-Series sources in one atomic bundle. The cross-family contract is documented in [`docs/v2/BUILDER_HIERARCHY_CREATION.md`](./docs/v2/BUILDER_HIERARCHY_CREATION.md); family-specific behavior remains in the focused [TMDB Lists](./docs/v2/BUILDER_TMDB_LISTS.md), [Decades](./docs/v2/BUILDER_DECADES.md), [People](./docs/v2/BUILDER_PEOPLE.md), [Franchises](./docs/v2/BUILDER_FRANCHISES.md), [Studios](./docs/v2/BUILDER_STUDIOS.md), [Networks](./docs/v2/BUILDER_NETWORKS.md), and [Genres](./docs/v2/BUILDER_GENRES.md) documents. The overall V2 Builder remains governed by its separate release and noindex boundary rather than replacing the stable root application.
...
The workspace **Import** action offers **Import from Nuvio**, **Import from file**, and **Import from JSON**. One modal retains method choices during compact acquisition, then hides them for shared full-width Review with header Back/source context and fixed footer Apply. File/paste drafts and the valid memory-only Nuvio session survive Back. Welcome and Send retain the standalone Nuvio dialog. File/paste stay local and review a detached snapshot before Apply. All three methods share Add, Merge exact matches and confirmed Replace when work exists. Merge offers **Keep existing artwork** (default), **Fill missing artwork** and **Prefer incoming artwork** field-by-field, without deleting usable existing artwork when incoming artwork is absent. Matched non-artwork settings and raw preservation remain authoritative; Welcome retains direct local open behavior.

...
A new React/Vite TMDB Collection Builder is being developed under `builder/`. The existing TMDB ID Lookup remains available at the site root, while `/builder/` provides the current development-preview welcome and collection-building interface. The Builder development preview includes a Blank-or-Decades launcher for New Collection and New Folder. Decades defaults to Both media and **Newest Decades, Oldest Years** Display order, separates content decisions in Configure Decades from counts, names, always-visible title/visibility outcomes, and shared collection/folder presentation controls in Review & Appearance, and retains full-preset current-Decade years, per-Decade Genre choices, deterministic planning, and physical-folder-aware names. This remains unreleased V2 work rather than a replacement for the stable root application.

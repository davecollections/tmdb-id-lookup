...
The workspace **Import** action offers **Import from Nuvio**, **Import from file**, and **Import from JSON**. File/paste stay local and review a detached snapshot before Apply. All three methods share Add, Merge exact matches and confirmed Replace when work exists. Merge offers **Keep existing artwork** (default), **Fill missing artwork** and **Prefer incoming artwork** field-by-field, without deleting usable existing artwork when incoming artwork is absent. Matched non-artwork settings and raw preservation remain authoritative; Welcome retains direct local open behavior.

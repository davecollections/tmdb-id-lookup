...
async function fetchCompanyDetails(id) {
  ...
  if (!res.ok) throw new Error(`Company details failed for ${id}: HTTP ${res.status}`);
  return res.json();
}

async function fetchMovieCount(id) {
  const res = await tmdbFetch(`https://api.themoviedb.org/3/discover/movie?with_companies=${id}`);
  if (!res.ok) throw new Error(`Movie count failed for ${id}: HTTP ${res.status}`);
...
function toCsv(companies) {
  ...
  const rows = companies.map(company => headers.map(header => csvEscape(company[header])).join(","));
...
if (OFFSET >= exportData.companies.length) {
  console.log(`Offset ${OFFSET.toLocaleString()} is beyond the export total of ${exportData.companies.length.toLocaleString()}. Nothing to scan.`);
  process.exit(0);
}
...
const stats = {
  ...
  offset: OFFSET,
  limit: LIMIT,
  actual_limit: selectedCompanies.length,
  checked: 0,
  found: 0,
  missing: 0,
...
console.log(`Manual rebuild: enriching ${selectedCompanies.length.toLocaleString()} companies from export offset ${OFFSET.toLocaleString()}.`);
...
const companies = Array.from(companyMap.values()).sort((a, b) => Number(a.id) - Number(b.id));

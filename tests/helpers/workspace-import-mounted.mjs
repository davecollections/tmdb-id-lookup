...
export async function runWorkspaceImportChecks(connection, origin, evaluate) {
	...
	for (const [width, height] of [[360, 800], [393, 852], [1280, 900], [384, 852], [402, 852], [412, 852], [899, 900], [900, 900], [901, 900], [393, 400]]) {
		...
		for (const stage of stages) {
			...
			if (width === 1280 || (width === 393 && height > 400 && ["methods", "merge-prefer"].includes(stage))) {
				if (stage === "merge-prefer" && width === 393) await evaluate(connection, 'document.querySelector(".merge-artwork-policies").scrollIntoView({ block: "start" })');
				await capture(`${stage}-${width}`);
				if (stage === "merge-prefer" && width === 1280) {
					await evaluate(connection, 'document.querySelector(".nuvio-merge-preview").scrollIntoView({ block: "end" })');
					await capture("artwork-counts-1280");
				}
			}
			...
			await evaluate(connection, 'new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))');
			if (stage === "nuvio") {
				assert.equal(await evaluate(connection, 'document.activeElement === document.querySelector("[data-action=open-nuvio-import]")'), true, "Nuvio returns to retained local surface");
				await key("Escape", "Escape", 27);
				await evaluate(connection, 'new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))');
			}
	...
	assert.deepEqual(external, [], "Local imports and unsubmitted real Nuvio login make zero external requests");

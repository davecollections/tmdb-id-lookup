...
One-sort default names are unchanged. Multiple sorts use friendly names such as Recent Movies and Top rated Series, retaining provider/region context where the containing Folder does not supply it. In selected-folder Add, issue #257 keys optional names by the complete existing DISCOVER recipe identity, including provider, region, media, sort and filters. A genuinely different candidate never inherits another name; the exact removed key may restore its dormant override in the same session. Guided naming and imported Source Edit contracts remain separate.
...
Selected-folder Add uses the shared, initially collapsed **Source names** disclosure. Each physical candidate has a generated effective name, immutable provider/region/media/sort context and optional final-title override. **Reset name**, blank or a generated-equivalent value returns to automatic. Invalid nonblank drafts remain recoverable and block Add. See [the shared Add naming contract](./BUILDER_SOURCE_ADD.md#optional-source-names-in-selected-folder-add-257); title remains excluded from DISCOVER recipe and duplicate identity.

Candidate order is user-selected region order, then Movie before TV within each region. Every concrete source retains this fixed identity:
...
Sources remain projection-free native TMDB `DISCOVER` sources with explicit `tmdbId: null`. The optional Advanced fields are `withGenres`, `withoutGenres`, `withKeywords`, `withoutKeywords`, `releaseDateGte`, `releaseDateLte`, `year`, `voteAverageGte`, `voteAverageLte`, `voteCountGte`, `withOriginalLanguage`, `withOriginCountry`, `withCompanies`, `withoutCompanies`, TV-only `withNetworks` and `withoutWatchProviders`. Required provider/region appear as fixed context; no additional included-provider control is offered. Excluding a required service blocks creation, naming every conflict in multi-service plans.
...
The hardened Streaming validator rebuilds expected candidates through the Streaming/DISCOVER constructors and enforces normalized provider identity, one or more distinct ordered selected regions, common availability, exact count, region order, Movie-before-TV order, unique comparable identities, shared semantic sort, a valid Nuvio title for every physical source, Provider ID, and the complete independently derived Advanced filter configuration. Custom titles do not relax any functional-source field.

## 9. Physical-source editing

The source menu offers the expanded Streaming editor for an unambiguous single-provider/explicit-region anchor. A defining Genre or canonical period overlapping that anchor routes to Full Discover because exported JSON cannot establish creation provenance. Compound provider expressions never imply a required service from token order.

The editor changes Source name, one evidenced sort and optional Advanced filters. Provider, Region and media remain fixed. Catalogue hydration is non-blocking and Use default name retains its existing behaviour. Unknown fields and unsafe optional groups remain preservation-only; touched canonical fields and proven equivalent mirrors alone are patched. Exact Preview follows the current effective draft through the broad Discover routes.
...
The framework-independent ephemeral planner creates only ordinary Collection, Folder, and canonical native Streaming DISCOVER source drafts including supported Advanced settings. It revalidates immediately before Apply. Display titles are never placement identity:

- grouped placement trusts only contained canonical supported Streaming evidence consistently identifying one provider;
...
Deferred work includes logical/bundle Streaming editing, Provider/Region/media replacement in existing bundles, runtime, provider AND/OR source semantics, generic cross-family multi-select, automatic provider-logo Folder artwork, title counts, persistent provider storage, automatic refresh, recipes, and broader Help work.

...
- Decades (#224) uses the existing strict canonical period classifier over every effective Source. One shared catalogue Decade plus actual Movie/TV media yields Movies, Series or Mixed artwork from a static exact-URL projection. Overview/year/Genre/multiple-sort and valid Advanced variants remain eligible after export/reopen and renames. Poster, Square and Landscape have matching cover/focus pairs with common hero/logo; parent covers remain excluded. See [the Decades contract](./BUILDER_DECADES.md#curated-folder-artwork-224).

Folder title, approximate text, and current image URL are never identity evidence. Mixed identities, unsupported source semantics, addon/opaque sources, and empty/mixed-purpose folders produce no suggestion. Franchise and Streaming sources also produce none because they have no approved Folder curated-artwork authority for this feature.

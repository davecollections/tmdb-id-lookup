...
These settings previews do not consult the artwork runtime, manifests, TMDB, folder titles, source identities, or sibling folders. They perform no URL normalization, host allowlisting, proxying, download, orientation detection, or replacement. Curated suggestions and identity inference remain deferred and would require separately approved authoritative evidence plus explicit acceptance; visible imported/custom values remain unchanged unless their own field is deliberately edited, while hidden compatibility values remain exactly preserved. Issue #134 itself still adds no Settings preview or suggestion behavior.

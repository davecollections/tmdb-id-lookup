...
Each curated action changes only its local editor draft field. Tile uses the current draft Poster/Landscape choice, and changing shape may change blank-field curated/request availability without rewriting `coverImageUrl`. Studio Poster is unsupported and therefore receives neither action. Choosing Focus changes only `focusGifUrl`, never `focusGifEnabled`. Cancel remains zero mutation; normal Apply commits only touched fields in one revision and retains raw/unknown import data. Guided People, Studio, Network, and Genre creation keeps its separate creation-time automatic artwork/fallback contract unchanged. Automatic shape switching, opposite-orientation substitution, missing-orientation warnings, sibling notices, bulk editing, Source Edit imagery changes, and exported provenance remain deferred. Any later automatic URL replacement may replace only a URL positively recognized as an exact curated asset; TMDB fallback, imported, custom, and unknown URLs are not replacement authority.

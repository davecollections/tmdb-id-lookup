...
**Implemented locally as the non-visible issue #112 foundation; issue #113 remains the visible follow-up:** **Decades** has one deterministic eight-preset catalogue from 1950s & Earlier through 2020s, injected current-year expansion, additive whole/year/Decade-level-Genre DISCOVER construction, exact duplicate/elsewhere planning, and concrete ephemeral New Collection/New Folder hierarchy plans. A selected Genre incompatible with Movies-only or Series-only rejects the complete configuration; Both retains Movie-only and TV-only concepts through their supported physical source. Both media defaults to complete separate Movie and TV collections and may instead use one mixed collection; plans carry real `TABBED_GRID`/`ROWS` presentation values, derive counts from actual bundles, and apply new collections through one bounded atomic controller operation. The maximum accepted configuration produces 452 sources across 2 collections/16 folders in one revision without a Year × Genre product. Canonical date-based classification is title-independent, requires `tmdbId` to be `null` or absent, and reserves Decade-period Genre recipes from the Genre editor. No launcher, modal, preview, blank-creation change, or visible Decade editor is included. See [`BUILDER_DECADES.md`](./BUILDER_DECADES.md).
...
**Confirmed future creation direction, for reassessment after the visible Decades flow:** issue #112 supplies only the bounded non-UI Decades hierarchy seam. A later `New collection` flow may let the user choose a content/source type—such as People, Studios, Networks, Franchise, or Streaming—then use the matching picker and automatically create the necessary collection, folder, and source structure. It must retain explicit **Start blank** / **Manual** routes. `New folder` may offer the analogous source-led shortcut inside an existing collection. Compatible pickers may support multi-select with sensible shared defaults and progressive **Advanced** per-item exceptions instead of forcing repetitive one-at-a-time configuration. Before migration, reassess the proven per-family flows and introduce only the smallest shared interaction work justified by those consumers; a broad speculative UI/interaction-foundation phase is not a prerequisite.
...
17. Decades — issue #112 implements the local, non-UI catalogue/source/hierarchy planning, duplicate/stale validation, canonical classification, and atomic multi-collection foundation. Issue #113 remains responsible for launchers, configuration/review UI, presentation controls, preview/status presentation, and visible Decade Source Edit.

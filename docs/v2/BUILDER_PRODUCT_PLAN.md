...
**Current main baseline:** `1cafa19168d23538d8ba416ed75cc52d3005ccb9`, verified clean and equal to `origin/main` for roadmap reconciliation [#210](https://github.com/davecollections/tmdb-id-lookup/issues/210). The [canonical roadmap](#18-roadmap-and-mandatory-gates) below owns current product direction; dated checkpoints and focused implementation documents retain their historical evidence, not outstanding release gates.
...
Last reviewed: 2026-09-13 (roadmap reconciliation #210; verified through #208 / PR #209)
...
- profile selection;
- how an intended Add / Merge / Replace experience can be provided over a documented full-replace API;
...
**Canonical roadmap and product-direction owner.** This section reconciles the owner-approved direction and retained repository plans as of #210. It is a dependency-aware direction, not a rigid release schedule: no dates, release allocation or implementation promises are implied. Each implementation needs its own focused owner-approved scope. Listing a candidate does not create an issue or select it as the next task. Technical contracts and historical evidence remain in their focused documents.

### Current state

The present focus is building collections plus strong preservation-first import/edit capability. V2 remains an isolated, unadvertised development preview under `/builder/`, with `noindex, nofollow`; stable V1 lookup/copy-ID/export remains supported.

The eight established hierarchy families, shared title Preview, ordinary physical-source editing, reordering and **Global display settings** are implemented. Global display settings (#154 / PR #155) is an explicit one-time operation over existing nodes, not persistent defaults. JSON export (#194 / PR #195), round-trip Source recognition (#196 / PR #197), multi-sort creation (#198 / PR #199), native variants/missing-source additions (#200 / PR #201), standalone Discover creation/safe editing (#202 / PR #203), and imported List editing/current-draft Preview (#204 / PR #205) are merged. Supported standalone Discover is editable; unsupported or ambiguous imports remain preserved and fail closed. Earlier statements that all generic Discover is non-editable or that export has yet to exist describe superseded stages.

The [current checkpoint](#current-checkpoint---2026-09-13) records completed #206/#207 and shipped Studio Minimum votes #208/#209, its exact main SHA, publication and evidence limits. **Studio Minimum votes is not in the future queue.** Current Preview still has family-specific sample limits; the up-to-100 expansion below is not implemented.

Open-issue inventory checked on 2026-09-13: this documentation issue [#210](https://github.com/davecollections/tmdb-id-lookup/issues/210); V1 screenshot/readme polish [#9](https://github.com/davecollections/tmdb-id-lookup/issues/9); public TMDB list lookup investigation [#14](https://github.com/davecollections/tmdb-id-lookup/issues/14); ongoing cache-audit reporting [#19](https://github.com/davecollections/tmdb-id-lookup/issues/19); and unsupported direct TV/season source tracking [#24](https://github.com/davecollections/tmdb-id-lookup/issues/24). None selects a new Shared Advanced implementation issue. Resolving known public List URLs/IDs is already implemented in Builder; that does not complete #14's keyword/name-search investigation. V1 screenshots and cache maintenance are not new Builder milestones.

### Next

The selected bounded continuation is [#216](https://github.com/davecollections/tmdb-id-lookup/issues/216): shared Minimum/Maximum rating for native Studio and Network creation and physical editing, extending the existing disclosure, preservation rules and exact Preview without Worker changes. The wider family/field choices below remain future decisions.
...
**MDBList remains desired after this prioritised Shared Advanced direction**, unless the owner changes that order; its scope and support gates are under Later.

### Later

These are retained product features and polish candidates, not a ranked release backlog.

- **Shared Preview expansion — approved implementation #226.** Supported exact TMDB sources expose the first 100 ordered source-result positions before poster filtering, with deliberate sequential paging through page five where applicable. Recognized List sorts apply separately within each page; complete People/Collection data needs no paging; representative Decade/Period samples remain bounded. See [the shared contract](./BUILDER_TITLE_PREVIEW.md). Worker deployment and production page-2 acceptance remain separate owner gates.

  Truthful completion/count states remain: established completion at or below 100, **“Showing all 18 titles”**; 100 represented positions with a known larger total, **“Showing the first 100 of 124 titles”**; unavailable/unreliable total at the cap, **“Showing up to 100 titles”**. Do not claim all titles or the first 100 before that many are actually represented; interim/loading and missing-poster wording remains truthful.

- **MDBList — later product candidate with support gates.** The retained proposal references [NuvioTV #2812](https://github.com/NuvioMedia/NuvioTV/issues/2812); its presence is not proof of a supported native Collection format or release date. An initial lookup/search-only trial, omitting or disabling Create Source, remains an undecided possibility. Before native creation/export, verify actual Nuvio support and serialized representation; do not guess provider fields, authentication or output format. It is not a dependency of multi-sort creation. Account/watch-history integration is outside this lookup proposal.
- **Quick Setup, templates and recipe engine — product features.** Build on reliable ordinary creation flows. Retain the four startup routes and meaningful return home in [Startup](#4-startup-experience), the short editable **Dave's 1-Click Setup** journey, and inspectable recipes that generate ordinary hierarchy data. Keep manageable defaults and searchable larger catalogues; exact recipe names, size, content, branching, ordering, regional/provider choices and device defaults remain [open decisions](#19-open-decisions). Neither these features nor Review/export depends on connected mode.
- **Review/export usability and export-help refinement — polish.** Improve the existing export journey, validation and preservation explanations; do not describe basic export as unfinished. Inside the existing expanded help, offer **Nuvio.tv / TV app / TV via phone or computer** method buttons switching one instruction panel. Retain the owner-requested same-network QR/full-address guidance and **Collections > Import > Paste or File > Import > Save Changes** sequence for TV management, subject to dated client verification when implemented. Explain that URL import requires an already hosted JSON file. Preserve shared beta and TMDB Enrichment guidance. **Copy JSON and Download JSON remain complete supported manual paths.**
...
- **Collection Folder management — #232 in local owner review.** Full Collection settings applies a chosen shape to existing child Folders once, with exact recognized curated artwork transitions and preservation otherwise. Collection **Remove folders** supplies retained ordered multi-selection, an always-visible irreversible-action warning and one atomic deletion; that dialog is the confirmation. Source/Collection bulk deletion, move/copy and generic transactions remain deferred. No persistent Folder default is introduced.
- **Folder sorting — #232 in local owner review.** **A–Z / Z–A** use current titles and V1 Name/title comparison; confident identity-based People Collections also expose **First name / Last name**. Last name uses V1's pragmatic final-word rule, including mononyms and multiword names, without structured-name metadata or enrichment. First name is retained even where equivalent to A–Z. Stable ties and unnamed-last behavior preserve predictability; one atomic permutation retains active selection and immediate manual drag. Sorting is not a persistent automatic mode.
...
- **Back to top / large-collection navigation — small polish.** Improve practical navigation for large imports, especially on mobile, without inventing a broader workspace redesign.
...
If adoption and continued development justify it, Dingo's may evolve toward a **“Nuvio collection-management companion”**: build, import, safely edit, reorganise and maintain large configurations, compare/merge configurations, potentially audit configuration health, and eventually connect to Nuvio optionally. This is a direction, not an immediate rename or implementation commitment. Keep **Dingo's Collection Builder** as the present name and creation plus preservation-first import/edit as the present focus. Nuvio remains the media centre and playback application; Dingo must not become its playback/media-centre replacement or add watch-history/general media-centre functions merely to broaden scope.

**Optional direct Nuvio import/export / Send to Nuvio — long-term capability.** The [dated connection evidence and gates](#16-optional-future-nuvio-connection) establish a transport to investigate, not a finished safe flow. Verify authentication/pairing, profile targeting, Add / Merge / Replace behavior, update targeting, token/security handling, disconnect/revocation, recovery and safe destructive-replacement handling before implementation. The core Builder must remain usable without login, a Nuvio connection, a personal TMDB API key or mandatory cloud storage. Manual JSON import, Copy JSON and Download JSON remain available even after connection exists.
...
- **Unsupported filters/source types** remain preservation-only where applicable. Runtime-length, vote-count maximum and other unmodeled fields need evidence before exposure; direct movie/series/season types must not be invented. Lists and Franchises do not inherit Discover controls.
...
This retained foundation ledger describes the contracts and acceptance boundaries at those issues' historical stages. It is not a second roadmap. Later #154, #194, #196, #198, #200, #202, #204 and #208 supersede earlier pending/non-editable/export-not-yet-available wording as summarized in [Current state](#current-state). Old client evidence does not become new acceptance by moving this ledger.

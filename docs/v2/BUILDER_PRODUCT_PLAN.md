...
Discover [#202](https://github.com/davecollections/tmdb-id-lookup/issues/202) is complete through merged [PR #203](https://github.com/davecollections/tmdb-id-lookup/pull/203), commit `d9ad329d761522735e964ca69fba8267fac095ed`. Imported List editing and current-draft Preview [#204](https://github.com/davecollections/tmdb-id-lookup/issues/204) is complete through merged [PR #205](https://github.com/davecollections/tmdb-id-lookup/pull/205), commit `efe9966ebb04392dbc540f131ff19790d1c1e8ca`. Owner review, automatic publication and cleanup are complete; the [Pages run for current main](https://github.com/davecollections/tmdb-id-lookup/actions/runs/34724507178) succeeded. The older checkpoints below describe their historical stage, not outstanding acceptance gates.

Investigation [#206](https://github.com/davecollections/tmdb-id-lookup/issues/206) is complete with accepted [capability findings](./SHARED_ADVANCED_ASSESSMENT.md) and preservation evidence. The owner authorized final commit/push, normal merge of PR #207 after passing checks/review, synchronized main and safe branch cleanup. The [Studio minimum-votes scope](./STUDIO_MINIMUM_VOTES_ISSUE_DRAFT.md) is approved for its own issue and dedicated branch; runtime implementation does not start in this closeout.

The later [saved-JSON pack](../../manual-tests/collection-preservation/README.md) forms part of the [combined PR inventory](../../manual-tests/collection-preservation/REVIEW.md#complete-combined-pr-inventory). The owner confirmed that every method started with a fresh import of the original master. Manage from phone imported and exported through that interface. Direct TV imported through the local network URL, exported on the TV, and the on-device export was retrieved using File Explorer. The compressed transfer decoded to the unchanged master. Exact run times, sync activity, destination isolation and other unrecorded procedural details remain unknown; no internal import/sync/export cause is inferred. The owner reported the preservation findings in Nuvio Discord; no message link was supplied. Builder creation and export must continue using the correct supported JSON, with no workarounds for upstream sort rewrites, removed exclusions or differing defaults. These findings do not block the approved Studio minimum-votes work. Private evidence remains outside Git; broader Advanced, People and compound locale work remains separate.
...
1. **Shared Advanced options, ahead of MDBList.** The #206 assessment recommends native Studio minimum votes first: one optional, unset control across Add Source, New Collection, New Folder and Source Edit, with exact filtered Preview and variant identity. Current native clients consume `voteCountGte`; six production query checks confirm the existing gateway path. The owner-supplied website pair preserves all six Studio/Network source objects, and Desktop 0.1.23-alpha (23) screenshots show different 0/100 results for Studio Movies, Studio Series and Networks, including five Studio Series titles at 100. This supports application in that build; no direct Desktop import/export or per-title vote audit is claimed. Focused scope and issue creation are approved. Coding starts in a dedicated implementation chat/branch from verified updated main, retaining PR #207 and accepted evidence. Broader client tests, People and compound locale work do not block that start. Then extend to Networks/Streaming and consolidate existing Genre/Decade controls before broader fields. Preserve defining constraints and accepted Both behavior. Lists and Franchises do not gain Discover controls. Use the approved draft to create the focused issue during #206 closeout.
2. **Native People remains unsupported for filter controls.** All three pinned native resolvers ignore filters while applying exact cast/Director-job and media selection. Tom Hanks zero/max-int32 fixture pairs visibly distinguish applied from ignored minimum votes; installed tests await Dave. Anne Hathaway's imported sources are Lists: matching alias removal with retained numeric values establishes preservation only. Do not convert source families or expose an ineffective control.
3. **Compound language/country support remains evidence-gated.** The original audit reproduces all 48 blocked Discover sources and exact preservation. Clients forward their strings, but the current production gateway rejects compound locale queries and field/media endpoint semantics remain unresolved. Keep existing editing validation; complete the independent Movie/TV language/country cases before a narrowly scoped validator, Preview and separately owner-deployed Worker change. Preserve literal expressions, aliases and types.
4. **Export-help refinement.** Inside the existing expanded export help, offer method buttons **Nuvio.tv / TV app / TV via phone or computer** and switch one instruction panel within the existing dialog. TV manager instructions should cover scanning the QR code or entering the full address beneath it on a device connected to the same network, then **Collections > Import > Paste or File > Import > Save Changes**. Explain that URL requires an already hosted JSON file. Preserve shared beta and TMDB Enrichment guidance.
5. **Nuvio.tv Most voted reproduction confirmed; historical attribution remains bounded.** The corrected September comparison uses the 2,619,074-byte original input, SHA-256 `e3baaefbb0de52639f5a7789a02989f74e9aa4827c6863a402dd50fc838818a7`. It matches 18 collections, 756 folders and 3,538 sources without identity/order changes and confirms the two reported Most voted fallbacks. All 18 collections already use ROWS; there is no layout change in this pair. The earlier comparison used a separate TABBED_GRID input version; both versions and provenance are preserved without attribution. Current deployed normalization code reproduces the same sort fallbacks in an isolated pure check. That identifies a mechanism, not the responsible step in the historical sequence. Dave subsequently supplied the exact six-source fixture and a 13 September export through Collections → Import → Collections → Export. Both Most voted rewrites are confirmed, while Top rated controls, numeric thresholds, collection settings and order survive. The referenced screenshot and exact build/sync metadata were not supplied here. Builder must continue exporting the selected `vote_count.desc`; do not imitate the website fallback. Keep Builder exports unchanged; an upstream report still requires separate authorization.
6. **Folder-level Focus Glow/pin assessment is bounded.** The September output removes 176 folder `focusGlowEnabled: true` values and one folder `pinToTop: true`. Neither field is present in the inspected native folder models or website folder serializer. Supported collection fields and folder `focusGif*` are distinct. Classify these as unsupported folder extensions, preserve imported data in Builder, and do not relocate or normalize them.

The completed #204 acceptance includes all four Preview corrections: current-draft Discover media/orders/effective filters, selected-only combinations, concise complete/partial List summaries, and removal of the technical Edit paragraph with conditional filter/sort notices retained. These are merged and published; they do not verify new #206 client capabilities. Shared Advanced runtime implementation, People changes and MDBList work have not begun.

Status: Durable product direction for the isolated v2 Builder

Last reviewed: 2026-09-13 (current checkpoint and priorities)

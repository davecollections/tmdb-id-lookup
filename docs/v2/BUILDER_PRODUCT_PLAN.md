...
**Current Builder capability:** browser-direct Import and reviewed Replace-only Send, with independent Download JSON and Copy JSON. Export & Send uses the final optional-backup, fresh-preflight and exact-readback contract; non-blocking preservation warnings are not shown. Shared protected-profile PIN verifies automatically at four digits. Active Send work is non-dismissible, progress is centered and verified success is explicit. The [connection contract](./BUILDER_NUVIO_CONNECTION.md) owns safety details and accepted limitations.

Owner-operated acceptance on a physical iPhone covered authenticated connection, protected-profile four-digit PIN auto-verification, one real replacement and exact readback verification. The owner independently confirmed the expected Collections were present and working in Nuvio, required no restoration, and repeated the complete mobile flow successfully after presentation polish. This is bounded evidence for the tested flow, not broad browser/device/client certification.

The [canonical roadmap](#18-roadmap-and-mandatory-gates) records the owner-approved next five, followed by a launch-readiness assessment. V2 remains isolated/unadvertised with `noindex, nofollow`; public-release approval is unchanged.
...
The current welcome screen supports starting a clean collection and a unified Import section with Nuvio, local-file and pasted-JSON methods. Quick Setup and templates remain future work; the four-route experience is not implemented and follows the canonical roadmap.
...
Trakt list lookup is approved future scope in the canonical roadmap. A possible future colourway does not expand that scope to watch history, tracking or account management.
...
**Owner-approved near-term sequence.** The Merge artwork policy and workspace Import work in #259 is implemented pending owner review and integration. The remaining four features are future / not implemented and each needs its own focused investigation and implementation task. Completed #242 and #244 work is recorded in Current state.

1. **Merge artwork policy + workspace Import — implemented in #259, pending owner review/integration.** Source-neutral Keep existing (default), Fill missing and Prefer incoming apply field-by-field without missing-incoming deletion. Workspace Import offers Nuvio/file/pasted JSON with detached review and common Add/Merge/Replace. The owner-review amendment embeds shared Nuvio stages in the same Import modal; Welcome and Send retain standalone entry. Authenticated embedded review and physical-phone acceptance remain required before merge. See [the import contract](./BUILDER_WELCOME_IMPORT.md#workspace-import-and-merge-artwork-259).
2. **Find / Jump — future / not implemented.** Search Collections, Folders and Sources and navigate directly to a result, selecting/navigating its parent hierarchy as needed on desktop and phone. Architecture and final interaction design remain for the feature task.
3. **Move Folder(s) between Collections — future / not implemented.** Move one or more Folders through an atomic Builder/controller operation, preserving contents and identity/data where safe, to support project reorganisation. Final UX and implementation remain open.
...
Later design should add unique Collections/Folders/Sources, review matching Collections/Folders instead of blindly flattening them, allow exact duplicate Sources to be deduplicated, and retain differently configured variants. Surface artwork/presentation conflicts and user-selectable outcomes; preserve unknown imported fields first. Matching rules, stable identity, ordering and conflict review require explicit design. Local connected Import already uses a conservative exact-name merge planner. This does not implement two-file comparison/conflict review; any reuse for that flow or templates needs separate design. This file-level flow remains distinct from explicit in-workspace Folder/Collection Merge above.

### Long-term direction

If adoption and continued development justify it, Dingo's may evolve toward a **“Nuvio collection-management companion”**: build, import, safely edit, reorganise and maintain large configurations, compare/merge configurations, and potentially audit configuration health. Optional read/import and Replace-only Send are implemented; the canonical Next sequence owns subsequent priorities. This is a direction, not an immediate rename or implementation commitment. Keep **Dingo's Collection Builder** as the present name and creation plus preservation-first import/edit as the present focus. Nuvio remains the media centre and playback application; Dingo must not become its playback/media-centre replacement or add watch-history/general media-centre functions merely to broaden scope.

Optional Direct Nuvio read/import is merged through #238 / PR #239 and Replace-only Send through #244 / PR #245. Remote Add/Merge/selected-Collection modes remain deferred. The current connection contract retains the accepted residual concurrency limitation; no broader synchronization product is implied.

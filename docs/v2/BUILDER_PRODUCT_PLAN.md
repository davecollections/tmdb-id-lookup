...
**Confirmed future creation direction, for reassessment after issue #113 owner acceptance:** the Blank-first ordered launcher registry now proves the smallest shared entry seam with Decades as its first automatic hierarchy consumer. Later modes may let `New collection` choose a content/source type—such as People, Studios, Networks, Franchise, or Streaming—then use its proven picker and automatically create the necessary collection, folder, and source structure. `New folder` may offer the analogous source-led shortcut inside an existing collection. Blank/manual creation must remain explicit and unchanged. Compatible pickers may support multi-select with sensible shared defaults and progressive **Advanced** per-item exceptions instead of forcing repetitive one-at-a-time configuration. Before adding a mode, reassess its proven family flow and add only the registry metadata and shared interaction work justified by that consumer; do not generalize the Decades-specific wizard into a broad speculative framework.
...
11. Native physical-source editing — integrated through issue #78 / PR #79 for Movie Collection and People, extended through merged Studio issue #92 / PR #93 and Network issue #98 / PR #99, then narrowly extended by issue #104 for simple Streaming-shaped DISCOVER and issue #110 for official Genre DISCOVER display-name/sort/approved-filter editing. Generic Discover remains intentionally non-editable. A complete current-client V2 edit/export round trip is deliberately deferred until V2 exposes export and is not an unfinished integration gate.
...
16. Genres — implemented on issue #110's focused review branch with all-27 ordered multi-select, current-folder or atomic sibling-folder-per-Genre destinations, exact-only official grouping, compact scalable review, shared responsive Advanced Add/Edit UI, DISCOVER Core construction/identity, and matching fail-closed source editing. Physical-iPhone review is complete; one short revised-phone confirmation and current-Nuvio fixture execution remain acceptance gates.
17. Decades — issue #112 implements the non-UI catalogue/source/hierarchy planning, duplicate/stale validation, canonical classification, and atomic multi-collection foundation. Issue #113 locally adds the shared Blank-first launcher, Both/Separate plus Newest-Decades/Oldest-Years visible defaults, three Display-order presets plus independent conditionally mixed-folder Source grouping, responsive per-Decade Genre configuration, physical-folder-aware naming, shared hidden-title accessibility, review/apply flow, shared presentation controls, scalable summaries, and narrow canonical Decade Source Edit; final owner acceptance remains its gate.
18. Reassess source-led creation using evidence from the completed source families and DISCOVER consumers.
19. Advanced Discover.
20. Broader deliberate V2 UI/UX and artwork-runtime integration at appropriate source stages.
21. Quick Setup, templates, and recipe engine after underlying creation flows are reliable.
22. Review/export usability.
23. Optional Nuvio connection only after its product, authentication, security, and replacement contract is verified.

...
18. Reuse-first hierarchy-family migration through the existing launcher — People is the first post-Decades proof through issue #118 / PR #119. Issue #120 codifies the shared standard. Franchises merged through issue #122 / PR #123, Studios through issue #124 / PR #125, and Networks through issue #126 / PR #127. The completed guided families are Decades, People, Franchises, Studios, and Networks.
19. Next likely hierarchy investigation — Genres, to close the hierarchy gap across the source families implemented so far. Discovery must begin by investigating and reusing issue #110's existing selected-folder Genre multi-selection and optional sibling-folder-per-Genre behavior. No design, issue, or implementation is approved by this direction.
20. After a future Genre hierarchy decision/implementation, pause new source-family expansion and plan a whole-Builder tightening/consolidation phase. Consider visual and Preview consistency—including the open mobile 10-poster/two-row question—accumulated UX rough edges, safe shared-code opportunities, evidence gaps, unnecessary duplication, and stale/deferred items worth closing. This is a future planning stage, not a pre-created issue list.

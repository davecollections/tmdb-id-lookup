...
2. **Back-to-top / large-library navigation.** This is the first product implementation item after reconciliation: a small navigation convenience, not a workspace redesign. Use one simple floating **Back to top** control, shown only after the user has scrolled a meaningful distance away from the top and hidden again once they return near the top. On desktop, prefer one workspace/page-level control; activating it returns the main Builder/workspace scroll surface to the top. On mobile, apply the same concept to the currently visible Builder/page scroll surface. Do not introduce per-column scroll-state bookkeeping unless the actual current layout architecture makes a global scroll target impossible. Preserve current scroll, focus and accessibility contracts.

   **Out of scope for this small feature:** search, jump-to-selected, alphabet navigation, minimaps, sticky hierarchy navigation, per-Collection/Folder/Source back-to-top controls, and a large-library navigation redesign.

   **Details for the focused feature issue:** scroll threshold, exact screen corner/offset, icon/text treatment, smooth versus reduced-motion behavior, and safe-area collision handling. The simple global intent is approved; these implementation details remain undecided.
...
| Back-to-top / large-library navigation | One simple floating workspace/page-level control is approved as the next product implementation item. The focused feature issue will decide scroll threshold, exact corner/offset, icon/text treatment, smooth versus reduced-motion behavior and safe-area collision handling within existing scroll/focus/accessibility contracts. |

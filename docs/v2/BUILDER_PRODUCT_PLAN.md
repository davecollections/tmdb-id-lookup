...
**Owner-approved near-term sequence. All five features below are future / not implemented.** This documentation records priority and intent only; each feature needs its own focused investigation and implementation task. Completed #242 and #244 work is recorded in Current state.

1. **Merge artwork policy — future / not implemented.** Offer an explicit precedence choice such as Keep existing artwork, Fill missing artwork from Dingo or Prefer Dingo artwork for matched Collections/Folders. A preferred side lacking artwork must not erase valid existing artwork. Exact UX, default and merge implementation remain open; today's existing-fields-win behavior is unchanged.
...
| Remaining management semantics | Folder removal and one-time sorting are implemented. Folder moves and Merge artwork policy are approved future features; their UX, artwork default and implementation remain open. Source/Collection multi-delete, copying, generic transactions and broader merge conflict outcomes remain deferred. |

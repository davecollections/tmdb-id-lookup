...
Investigation [#206](https://github.com/davecollections/tmdb-id-lookup/issues/206) provides the [shared Advanced capability assessment, pinned evidence and staged implementation plan](./SHARED_ADVANCED_ASSESSMENT.md), with [isolated manual tests](../../manual-tests/shared-advanced/README.md). Shared Advanced options take priority over MDBList. The owner accepted the findings and scoped acceptance evidence and authorized committing, pushing and opening the investigation PR. Runtime implementation, merge and deployment remain outside that authorization. The [proposed Studio follow-up issue](./STUDIO_MINIMUM_VOTES_ISSUE_DRAFT.md) is prepared for review.
...
1. **Shared Advanced options, ahead of MDBList.** The #206 assessment recommends native Studio minimum votes first: one optional, unset control across Add Source, New Collection, New Folder and Source Edit, with exact filtered Preview and variant identity. Current native clients consume `voteCountGte`; six production query checks confirm the existing gateway path. The owner-supplied website pair preserves all six Studio/Network source objects, and Desktop 0.1.23-alpha (23) screenshots show different 0/100 results for Studio Movies, Studio Series and Networks, including five Studio Series titles at 100. This supports application in that build; no direct Desktop import/export or per-title vote audit is claimed. The remaining requirement to start the Studio slice is focused implementation scope/issue approval and a dedicated branch from verified updated main, retaining the investigation PR as evidence. Broader client tests, People and compound locale work do not block that start. Then extend to Networks/Streaming and consolidate existing Genre/Decade controls before broader fields. Preserve defining constraints and accepted Both behavior. Lists and Franchises do not gain Discover controls. The proposed implementation issue is drafted but has not been opened.

...
**Future Franchise naming direction:** preserve meaningful canonical TMDB Collection wording, including semantic suffixes such as `Collection`, `Saga`, and `Trilogy`. Do not strip a trailing ` Collection` or reduce names such as `Matrix Collection`, `Avatar Collection`, or `Die Hard Collection` to ambiguous movie-like folder names. A future focused Franchise issue must inspect real TMDB Collection names and choose the least surprising display behavior. Removing a leading `The ` may be acceptable when the meaningful suffix remains, but it is not a universal cleanup rule; sort may instead ignore a leading article without changing the visible canonical title. No Franchise behavior is implemented by this plan entry.
...
18. Reuse-first hierarchy-family migration through the existing launcher — People is the merged first post-Decades proof through issue #118 / PR #119. Issue #120 codifies the shared standard before another family begins; other families remain future work.

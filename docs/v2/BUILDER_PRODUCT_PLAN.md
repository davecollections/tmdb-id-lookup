...
13. Network Series source creation and physical-source editing — implemented on issue #98's review branch using cached discovery that completely ignores legacy `t`, one automatic informational on-demand Series Count, exact projection-free `NETWORK/TV` construction, folder-aware duplicate handling, and display-name/sort-only edits. The tracked Worker change is not a deployment claim.

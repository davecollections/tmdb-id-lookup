...
1. **Next substantial investigation: shared Advanced options.** Reuse the completed #202 Discover foundation for applicable creation and editing families. Investigate votes, ratings, dates, genres, languages, countries, keywords, runtime and availability, mapping TMDB capability, Nuvio serialized fields and actual client consumption before offering each control. Preserve Studio, Network and period constraints. Lists and Franchises do not gain Discover controls. Warner Bros. Top rated correctly returning one-vote ratings motivates configurable options, not a hidden threshold. This broader work is separate from #204's preservation correction.
2. **People minimum-vote behavior and Advanced capability.** Investigate native PERSON/DIRECTOR credits consumption independently: the inspected resolver does not apply Discover filters. Classify by the physical source type, not the folder name: TMDB Lists stored inside Actors folders remain LIST sources and do not establish native People behavior. Preserve exact Acting/Directing roles and media choices; do not silently convert People into another source family or promise that saved filters affect results. Establish unchanged import/export preservation of minimum-vote fields and actual filtering in the tested client before adding controls. List alias preservation and the List Preview filter notice are separate evidence, not proof of native People filtering. This is a deferred compatibility investigation; no People implementation is authorized in #204.
3. **Compound language/country support.** Investigate the 48 imported International Cinema Discover sources with pipe-separated language and country expressions. Verify both field contracts, client consumption, lossless editing and exact Preview semantics. Keep their current validation boundary in #204; adding List preservation does not establish Discover support.
...
5. **Unchanged Nuvio.tv round-trip reproduction before attribution.** The owner reports that the supplied round trip changed **Family Movie Night All-Stars** from **Most voted to Popular**, and **Biggest Reality Hits** from **Most voted to Recent**. These observations do not yet establish which step caused the change. Reproduce a clean import followed by an unchanged export; retain exact input/output, source IDs/types, client version and steps, and compare the physical sources before attributing the rewrites to Nuvio.tv. Use the existing website, synced-TV, direct-TV, Desktop and manually created Top rated comparisons as contextual evidence only. Keep Builder exports unchanged; do not imitate the unexplained rewrites. An external bug report remains on hold pending reproduction and separate owner authorization.
6. **Folder-level Focus Glow and pin schema compatibility.** Assess the reported removal of folder-level Focus Glow and pin settings against the supported Nuvio schema and actual client consumption. Identify the exact imported keys, values, types and hierarchy level before deciding whether they were valid folder settings. Do not assume a collection-level capability is valid on folders, or classify removal as client data loss before that contract is established. Keep raw imported data preserved in #204; do not add, remove, relocate or normalize exported fields to match an unexplained round trip. Investigation is recorded only; no schema or presentation implementation starts here.

Owner-review checkpoint, 2026-09-13: all four requested #204 Preview corrections are already implemented locally: current-draft Discover media/orders/effective filters, selected-only combinations, concise complete/partial List summaries, and removal of the technical Edit paragraph with conditional filter/sort notices retained. Compatibility investigations above remain separate from #204 acceptance. Commit, push, PR creation and deployment stay on hold while the latest Preview changes are reviewed. Shared Advanced options, People implementation and MDBList work have not begun.

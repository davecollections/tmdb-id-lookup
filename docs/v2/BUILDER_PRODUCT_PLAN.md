...
- **Shared Preview expansion — product feature.** Show useful initial content quickly (approximately ten posters), append/reveal more through lazy vertical scrolling, fetch further pages only when needed, and stop at **100 titles**. Share the pattern across pageable Preview families where technically supported, retaining each family's exact draft/media semantics. This is separately scoped future work and may require approved Worker/page-range changes and manual owner deployment; it is not a Lists-only extension or a request to implement pagination now.

  Truthful completion/count states remain: known total at or below 100, **“Showing all 18 titles”**; known total above 100, **“Showing the first 100 of 124 titles”**; unavailable/unreliable total, **“Showing up to 100 titles”**. Do not claim all titles or the first 100 before that many are actually represented; interim/loading and missing-poster wording must remain truthful when the feature is designed.
...
| --- | --- |
| Next Shared Advanced slice | Network Minimum votes #213 is selected and awaiting owner review; subsequent Streaming/Genre/Decade scope and additional fields remain undecided. |

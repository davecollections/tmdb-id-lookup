...
- **Bulk actions — reusable management infrastructure.** The first obvious need is deleting multiple Sources, Folders or Collections with a reviewable scope and safe confirmation. Reuse selection/controller foundations; future move/copy actions could build on that infrastructure but are not promised together. Existing Bulk display settings does not mean bulk delete/move/copy is implemented.
- **Folder sorting — explicit one-time action.** Candidate sorts are **A-Z**, **Z-A**, **People by surname** and **People by first name**. Name interpretation and scope need design. Manual drag ordering remains available immediately afterward; sorting is not a persistent automatic mode.
...
5. Bulk presentation settings were a separate future issue at this stage; the current Bulk display settings feature subsequently merged through #154 / PR #155.

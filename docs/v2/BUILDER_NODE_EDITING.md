...
Visible Collection settings use the same compact semantic sections as Folder settings. **Basic details** contains Title. **Display** contains Hide collection title in Nuvio, Tabs/Rows layout, Include an All tab when using Tabs, and Pin to top. **Artwork** contains the optional `backdropImageUrl` control labelled **Backdrop Image or GIF URL** with the helper **Used as fallback folder artwork in Modern View.** The source-presentation field uses the heading **How sources appear in this collection**, the helper **Choose how each folder in this collection displays its sources in Nuvio.**, and the visible label **Tabs (recommended)** for the unchanged `TABBED_GRID` value. `focusGlowEnabled` remains a recognised collection compatibility field but is not rendered as a user-facing setting.

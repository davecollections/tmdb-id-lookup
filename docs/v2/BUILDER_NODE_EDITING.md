...
The UI binds immutable opening Project/Collection references separately from the bounded editor draft. On explicit shape Apply, `applyCollectionFolderShape` prepares exact-authority artwork suggestions, re-resolves the current Collection and checks the opening references again before building patches from current child values. Content, membership, ordering, project or active-draft changes reject the complete operation with an actionable error. Selection-only notifications are harmless. The existing `applyPresentationUpdates` commits the Collection settings and all child shape/eligible artwork patches together, once; effective no-ops advance no revision. Ordinary settings without a child shape operation keep the existing simple editor path.

`planCuratedFolderShapePatch` is shared with ordinary Folder settings. Exact current Tile and Focus URLs transition independently only to published counterparts for the same source-derived authority; custom, unknown, fallback and missing-orientation URLs stay byte-exact. Hero, video, logo, emoji, title, Focus enabled and source fields never participate. Authority failure preserves URLs while allowing shape changes. Collection settings shows only the two quiet scope/artwork helper sentences, without per-folder warnings or a conversion report. See [artwork authority](./ARTWORK_RUNTIME.md#collection-shape-transitions-232).

Permanent saved-content removal uses **Delete**. Single-node and bulk Folder confirmations place **Cancel before Delete** in DOM and visual order, with Cancel initially focused. Escape cancels, Cancel restores the trigger, and successful deletion focuses surviving content. Pending unsaved selections may still use Remove. Existing empty-node shortcuts gain no extra confirmation.

**Delete folders** is the confirmation surface, capped at 560px on desktop with compact 48px-minimum rows. It lists current-order safe titles plus position/source count, visible selected count, Select all, Clear all, Cancel and a red **Delete N folders**. The always-visible shared amber attention notice says: “This can’t be undone. Selected folders and all sources inside them will be permanently deleted.” Native full-row checkboxes keep the shared green retained-selection style. Select all selects the complete ordered list; Clear all clears the whole set. Search, filters and All/Selected tabs are absent. No second confirmation, thumbnails, lookup, selection cap or virtualization is added. The controller removes all selected subtrees atomically. Unaffected selection survives; a removed Folder or selected descendant recovers to the next surviving sibling, previous sibling, then Collection. Focus follows the surviving selection/nearest Folder or empty Add action, with mobile returning to Folders. Cancel/Escape restores the exact entry trigger without content mutation.

**Sort folders** is a one-time order operation in a compact dialog capped at 460px on desktop and phone. The existing management shell uses natural content height, safe Visual Viewport margins and centering, with a bounded inner scroller only when needed. Shared body lock, focus containment, Escape and trigger restoration remain unchanged. A–Z/Z–A use current titles and the existing V1 Name/title precedent: trim, strip leading The/An/A, preserve letters/numbers/whitespace/apostrophes/hyphens, tokenize other punctuation, and compare with `localeCompare` base sensitivity. Equal keys stay stable; hidden/blank/unusable names remain last in both directions. A confident People-only Collection also offers **First name** and **Last name**. Every Folder must be nonempty and contain only supported native PERSON/DIRECTOR sources for one positive TMDB person ID, resolved through the existing canonical People identity contract. Valid preserved provider/type/media casing and numeric-string IDs are accepted; eligibility never rewrites imported values. Mixed, addon, opaque, empty or conflicting identities omit People options. Last name deliberately moves the final word first, including Jr. and multiword names; it makes no structured-surname claim. First name retains V1 name/title order even where equivalent to A–Z. Titles are never rewritten or enriched.

Builder deliberately mirrors the small pure V1 comparator instead of importing its classic DOM-owning script or changing V1 loading. Parity tests execute the actual V1 functions for articles, accents, punctuation, numeric words, apostrophes, hyphens, mononyms and the approved People examples. Sorting submits one complete permutation through `reorderFolders`; selection/identities remain stable and ordinary pointer drag works immediately afterward. Neither sort mode nor metadata is persisted.

Reuse map: existing `NodeEditor`/draft/action helpers, `FolderShapeChoices`, source-derived artwork authority and cached clients, ordinary Tile/Focus planners, atomic presentation updates, `HierarchyActionsMenu` placement/focus, safe `nodeTitle`, `GenreSelectionToolbar`, shared checkbox/selection CSS, modal focus and Add Source Visual Viewport/body-lock lifecycle, domain sibling replacement and controller recovery conventions. The two narrow domain/controller operations and one collection-scoped dialog supply semantics absent from the existing single-node delete/move and global display settings; they do not introduce generic transactions or a management dashboard.
...
Visible Collection settings use the same compact semantic sections as Folder settings. **Basic details** contains Collection name. **Display** contains Hide collection title in Nuvio, Tabs/Rows layout, Show All tab, and Pin to top. **Artwork** contains the optional `backdropImageUrl` control labelled **Backdrop Image or GIF URL** with the helper **Used as fallback folder artwork in Modern View.** The source-presentation field uses the heading **Collection layout**, the helper **Choose how each folder in this collection displays its sources in Nuvio.**, and the visible label **Tabs (recommended)** for the unchanged `TABBED_GRID` value. `focusGlowEnabled` remains a recognised collection compatibility field but is not rendered as a user-facing setting.

Folder settings use compact semantic groups. **Basic details** contains Folder name. **Display** contains one compact native-radio Folder title visibility group followed by Poster/Square/Landscape visual selection cards. **Artwork** contains four internal groups: **Tile** (`coverImageUrl`), **Hero / Background** (`heroBackdropUrl`), **Branding** (`titleLogoUrl`), and **Focus** (`focusGifUrl`, `focusGifEnabled`). **Hero / Background** conditionally adds `heroVideoUrl` compatibility UI only when the opening draft contains a supported nonblank value. Known `coverEmoji` remains supported and exactly preserved by the Folder model and creation flows, but is not exposed in Folder Settings. The three visibility choices present the complete outcome instead of exposing two interacting switches:
...
- the title, visible exact artwork URL drafts, and the presentation values relevant to that node type;
...
Collection `backdropImageUrl` is optional and has no manual or automatic default. A supported imported string appears exactly in the local draft; blank remains blank; absence remains absent in the project; and unusual non-string imported values remain hidden and preserved until the user deliberately enters a string replacement. The compact wide preview uses the shared exact current-draft image/GIF contract: decorative lazy/async native image loading, `no-referrer`, no host allowlist, no URL rewrite, no proxy, no dynamic lookup, and exact-URL-scoped nonblocking failure. Blank drafts render no preview or placeholder. Collection workspace cards stay text-only because current owner evidence identifies this value as a Modern View fallback for Folder surfaces rather than Collection-card artwork.

**Manually confirmed in Nuvio — owner-observed current-client behavior, 2026-08-22; exact client build not captured:** in Modern View, missing Folder cover and hero/background surfaces fell back to the Collection backdrop. Explicit Folder `coverImageUrl` and `heroBackdropUrl` each took precedence for their corresponding surface. Folder video behavior remained separate and later took over the focused hero experience in the tested case without establishing guaranteed timing. No visible effect was observed in Grid or Classic View. These bounded observations are not a timeless universal client contract.
...
The tile choices are Poster (`POSTER`), Square (`SQUARE`) and Landscape (`LANDSCAPE`), displayed as compact visual selection cards with CSS-only aspect-ratio previews, native checked state, a structural selected inset and full-card focus. Square is a normal authored choice. Untouched imported casing stays exact; Follow Layout and genuinely unknown values retain preservation guidance until a deliberate replacement. A missing curated orientation retains the current artwork with the existing notice; generic folders need no curated asset to use Square.
...
Manual blank Collection creation deliberately omits `backdropImageUrl`; later Folder artwork never populates it automatically.
...
- `data-settings-section="basic-details|display|artwork"` (full Collection and Folder settings)
- `data-artwork-group="tile|hero-background|branding|focus"`
- `data-editor-field="hideNuvioTitle|folderTitleVisibility|viewMode|showAllTab|pinToTop|backdropImageUrl|tileShape|coverImageUrl|heroBackdropUrl|heroVideoUrl|titleLogoUrl|focusGifUrl|focusGifEnabled"` (`hideNuvioTitle` and `backdropImageUrl` are collection-only; the folder visibility/artwork fields are folder-only; `heroVideoUrl` is conditional existing-value compatibility UI; preserved `coverEmoji` has no visible Settings marker)
- `data-artwork-preview="backdropImageUrl|coverImageUrl|heroBackdropUrl|heroVideoUrl|titleLogoUrl|focusGifUrl"`

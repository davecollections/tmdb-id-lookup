...
`DiscoverValueField` is extracted from `AdvancedDiscoverControls`; `advanced-discover.js` now permits a bounded field subset and owns shared touched-filter tracking. The existing imported-filter helper owns the extracted minimal touched patch. Studio's thin `studio-advanced.js` adapter owns its single-field subset, imported editability and fixed-entity request mapping. There is no second numeric validator, settings store, Preview shell, requester or planner.

...
**Requirement before the first implementation:** approve one focused native Studio minimum-votes implementation issue/scope, then create its dedicated branch from verified updated main under the repository workflow. The earlier blanket installed-client acceptance prerequisite is satisfied for this bounded Studio decision by the supplied Desktop evidence. A direct Desktop round trip, per-title vote audit, TV/Mobile acceptance, People tests and compound-locale investigation are not additional prerequisites to start it; they remain limits on broader claims or separate work. The [proposed follow-up issue](./STUDIO_MINIMUM_VOTES_ISSUE_DRAFT.md) is prepared for review. Publishing this investigation does not start runtime work or create the implementation issue.

## Implementation stages and Worker boundary

1. **Recommended first issue: native Studio minimum votes with shared Advanced control.** One optional integer control, unset by default, in the four existing Studio contexts. Reuse the Discover numeric input/error behavior and validation; keep COMPANY/ID/media/sort and grouping. Extend effective-filter candidate/variant identity and Preview request projection before enabling the UI. Use the existing `/builder/discover/movie` and `/builder/discover/tv` contract via a thin native query adapter. Revalidate exact source patches and plans; no hidden defaults and no standalone-Discover conversion. Website preservation and Desktop visual evidence now support starting this slice after focused scope/issue approval. Its implementation tests must cover unset versus explicit zero/100, invalid integers, exact saved filters, current-draft Preview/query/cache and duplicate identity, untouched imported fields, minimal edits, and all four creation/editing contexts through the approved live integration path.
...
### Complete scoped Git inventory

The final review contains **14 files**: the original 13 investigation files plus the requested proposed Studio issue. The three pre-existing document changes are included as well as every new file; directory summaries are not a substitute for this inventory.
...
No production files, Worker, dependencies or existing test harness changed. The owner accepted the investigation and authorized its commit, push and PR; use the published PR head to identify the final Git snapshot, keeping the baseline and private evidence hashes distinct. No unrelated changes, runtime implementation, merge, deployment or upstream report. The recommended next issue is the [prepared Studio minimum-vote draft](./STUDIO_MINIMUM_VOTES_ISSUE_DRAFT.md); it has not been opened.

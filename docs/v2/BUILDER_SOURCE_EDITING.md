...
Addon-backed, opaque/community, Discover, List, Company, Network, and other native source shapes remain readable, movable, preservable, and removable, but do not expose Edit. The editor never treats several sources as one logical bundle.
...
- `tests/builder-source-edit-foundation.test.mjs` covers adapter recognition, exact targeting, automatic/custom/default titles, Collection title adoption/reset/Cancel, all supported identity changes, verified sort inventory and exact patches, imported-value preservation, shared count cache/semantics/failure/Retry, duplicates, no-op/one-revision behavior, stale conflicts, ordering, and deterministic second cycles.
- `tests/builder-source-edit-ui.test.mjs` covers menu exposure/order, Source name and helper copy, immediate prepopulation, friendly count states, enabled Save after count failure, fixed person identity, verified/imported sort presentation, name-first Collection presentation, prominent duplicate alert presentation, explicit picker activation, inert/portal/focus lifecycle, dismissal, announcements, natural-height desktop sizing, attached footer/sort visibility, and full-height mobile responsive contracts.

...
Status: foundation merged through issue [#78](https://github.com/davecollections/tmdb-id-lookup/issues/78) / PR [#79](https://github.com/davecollections/tmdb-id-lookup/pull/79); Studio Movie/Series and Network Series name-and-sort adapters added on the issue [#92](https://github.com/davecollections/tmdb-id-lookup/issues/92) and [#98](https://github.com/davecollections/tmdb-id-lookup/issues/98) review branches. A complete current-client V2 edit/export round trip is deliberately deferred until V2 exposes export and was not an unfinished merge gate.
...
Addon-backed, opaque/community, Discover, List, incomplete Company/Network, and other unsupported native source shapes remain readable, movable, preservable, and removable, but do not expose Edit. The editor never treats several sources as one logical bundle.
...
- `tests/builder-source-edit-foundation.test.mjs` covers adapter recognition including Network, exact targeting, automatic/custom/default titles, Collection title adoption/reset/Cancel, all supported identity changes, People/Studio/Network sort inventories and exact patches, imported-value preservation, shared count cache/semantics/failure, duplicate handling, no-op/one-revision behavior, stale conflicts, ordering, and deterministic second cycles.
- `tests/builder-source-edit-ui.test.mjs` and `tests/builder-network-ui.test.mjs` cover menu exposure/order including Studio and Network, Source name and helper copy, immediate prepopulation, friendly count states, enabled Save after count failure, fixed People/Studio/Network identities, verified/imported sort presentation, logo/link/automatic current counts without Retry or Refresh, name-first Collection presentation, prominent duplicate alert presentation, explicit picker activation, inert/portal/focus lifecycle, dismissal, announcements, natural-height desktop sizing, attached footer/sort visibility, and full-height mobile responsive contracts.

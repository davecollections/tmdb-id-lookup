...
Workspace **Import** (#259) offers Nuvio/file/pasted JSON beside unchanged **Export & Send**. It reuses Welcome method cards/forms and shared Nuvio/local snapshot review. Add/Merge/Replace appear when the Collections-or-dirty rule finds work; Merge alone shows artwork policies and separate field counts. Local JSON stays detached until Apply. The native file input remains mounted while the local surface suspends for Nuvio, retaining one active trap/body lock and exact method-trigger return. Cancel restores workspace Import. See [the import contract](./BUILDER_WELCOME_IMPORT.md#workspace-import-and-merge-artwork-259).

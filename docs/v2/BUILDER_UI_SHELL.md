...
- Independent boolean settings normally keep a visible conventional checkbox or switch in a neutral container. The owner-approved Decades content choices — Decade overview, Individual years, and Genre breakdown — use independent full-card pressed buttons with `aria-pressed`, the established cyan surface/border/inset, and native Space/Enter activation. They keep Individual years as the sole default and prevent deselecting the final choice; wide layouts use three equal columns, intermediate layouts may wrap to two, and phones stack. Forced colours use a double selected border because browsers suppress box shadows in that mode.

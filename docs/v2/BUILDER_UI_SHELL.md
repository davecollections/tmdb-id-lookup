...
Workspace **Import** (#259) offers Nuvio/file/pasted JSON beside unchanged **Export & Send**. It reuses Welcome method cards/forms and shared Nuvio/local snapshot review. Add/Merge/Replace appear when the Collections-or-dirty rule finds work; Merge alone shows artwork policies and separate field counts. Local JSON stays detached until Apply. Nuvio runs inside the selected method panel through `NuvioImportFlow`; the outer Import modal alone owns focus containment, body locking, viewport and Close/Cancel. There is no suspended Import shell or second workspace Nuvio dialog. Native file/paste drafts survive switching, which cancels Nuvio review authority without disconnecting the valid session. Cancel restores workspace Import. Welcome and Send's Merge instead retain the shared standalone Nuvio shell. See [the import contract](./BUILDER_WELCOME_IMPORT.md#workspace-import-and-merge-artwork-259).

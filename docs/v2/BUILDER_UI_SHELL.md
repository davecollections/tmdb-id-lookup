...
The current controls remain blank/manual creation only. The confirmed future direction is a separate, later UI-foundation-led flow in which `New collection` or `New folder` can begin from a content/source type, open its compatible picker, and generate the predictable hierarchy automatically, while retaining explicit **Start blank** / **Manual** routes. Compatible pickers should eventually support multi-select with shared defaults and progressive per-item exceptions. Issue #92 documents this direction but does not implement or migrate these controls.
...
Creation, menus, quick rename, deletion, Add Source, and source editing share the existing navigation/pointer gate. They are disabled or ignored while settings, Add Source, source editing, delete confirmation, return confirmation, another menu, or a pointer reorder session—including its 150ms settle—exists. Opening Add Source or Edit source closes the active menu and exits keyboard reorder mode.

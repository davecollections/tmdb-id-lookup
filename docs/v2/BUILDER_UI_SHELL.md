...
Workspace **Import** (#259) offers Nuvio/file/pasted JSON beside unchanged **Export & Send**. Compact **Acquire** reuses Welcome method cards/forms with desktop columns and phone stacking; Nuvio Connect/profiles/PIN use the shared `NuvioImportFlow`. After acquisition, **Review** hides methods/divider and uses the full internal modal width, with header Back/source context/Close, one body scroll owner, and fixed Cancel/Import to Dingo footer. Add/Merge/Replace and Merge artwork cards remain stacked at every width. Replace keeps the same modal with safe/destructive footer actions; Back or Keep current work retains Review. The outer Import modal alone owns focus containment, body locking and viewport. Local JSON stays detached until Apply; Review Back restores the exact method and drafts/session while invalidating snapshot authority. Welcome and Send's Merge instead retain the shared standalone Nuvio shell. See [the import contract](./BUILDER_WELCOME_IMPORT.md#workspace-import-and-merge-artwork-259).

Issue [#232](https://github.com/davecollections/tmdb-id-lookup/issues/232), merged through [PR #233](https://github.com/davecollections/tmdb-id-lookup/pull/233), adds the Collection-only menu sequence **Edit / Sort folders / Delete folders / Delete collection**, one-time existing-child Folder shape in full Collection settings, and focused Sort/Delete dialogs. The mobile pencil stays rename-only. The dialogs reuse menu placement, safe titles, native selection semantics, focus trapping, inert workspace, Visual Viewport sizing and body lock; Delete keeps its amber irreversible-action notice, selection count, Select all/Clear all and actions outside its single scrolling list, without Search or tabs. Delete keeps its reviewed larger phone presentation and 560px desktop cap. Sort stays compact at every width with a 460px cap, natural content height, safe 12px-minimum Visual Viewport margins and centering. Its two/four choices wrap without shrinking text; only Sort receives this presentation variant. See [the authoritative management contract and reuse map](./BUILDER_NODE_EDITING.md#collection-folder-management-232-local-owner-review).

At short landscape heights, the compact Sort body can scroll while its heading and actions remain reachable. The approved Remove presentation is unchanged. Cancel/Sort completion restores the Collections level before focusing its trigger when the viewport has crossed from desktop to phone; deletion returns to Folders.
...
A jump re-resolves the exact ID in the current project using the existing hierarchy locator before calling `controller.selectNode`. Missing targets leave Find open with unavailable feedback and no controller operation. Successful selection exposes parents, closes Find, scrolls the target's primary card control with Find-specific `block: "center"` and the shared reduced-motion behavior, then focuses it without additional scroll. Browsers clamp centering at the end of the available scroll range; mounted checks cover both central targets and final-item visibility with lower-edge clearance. Ordinary non-Find card scrolling is unchanged. The existing mobile override reveals the result's own Collections/Folders/Sources level, and ordinary subsequent selection/back navigation clears that override. No editor opens and content revision/dirty state remain unchanged. Opening, typing, clearing, scrolling and cancelling preserve the entire controller snapshot.
...
Every collection and folder uses one shared visual card ordered as a 46px six-dot grip, flexible selectable title/details/navigation body with chevron, and a 46px vertical-dots overflow trigger. Sources use grip, flexible identity body, and the same overflow trigger without a chevron. Collection menus contain Edit, Sort folders, Delete folders and Delete collection; Folder menus contain Edit and Delete. A source resolved by the fail-closed editor registry contains Edit source then Delete; every unsupported native, addon, or opaque source menu contains Delete only. The former external Edit/Delete rail is absent. The grip alone begins dragging, the card body remains navigation, and the trigger is an independent sibling control inside the same visual background. Single-item handles stay internally aligned but disabled. Long titles use safe display fallbacks and a bounded two-line treatment.
...
- teal/cyan means single selected, active, or current; green means retained/additive multi-selection and separately scoped success; amber/orange is warning or needs review; red is error or destructive; muted grey is disabled or unavailable;
...
- Poster/Square/Landscape shape choices follow the same radio rule. Selection also strengthens the schematic shape preview, without adding a tick.
...
- A multi-select entity card uses a native checkbox for semantics and checked state, visually hides it, and gives the complete label/card a green selected surface/border/inset and its own focus ring. It does not add a circular checkbox substitute or decorative tick.
- Independent boolean settings normally keep a visible conventional checkbox or switch in a neutral container. The owner-approved Decades content choices — Decade overview, Individual years, and Genre breakdown — use independent full-card pressed buttons with `aria-pressed`, the shared green multi-selection surface/border/inset, and native Space/Enter activation. They keep Individual years as the sole default and prevent deselecting the final choice; wide layouts use three equal columns, intermediate layouts may wrap to two, and phones stack. Forced colours use an even inset border independent of the outer keyboard-focus outline; it does not change control dimensions.
- Semantic Include/Exclude takes precedence over ordinary single/multiple cardinality: selected Include mode and included items retain established green; selected Exclude mode and excluded items retain red with dashed borders. Without these stronger semantics, retained/additive multi-selection defaults to green, and ordinary exclusive/current choices use teal/cyan. Match any/Match all remains cyan; disabled/unavailable choices remain grey. Apply meaning, not HTML input type. Independent booleans stay neutral, and membership/action rows (such as resolved TMDB Lists and selected summaries) do not acquire fake toggle semantics. Success may independently retain scoped green: not every green element is selectable. Selected state must remain structurally distinguishable in grayscale and forced-colours mode; hue alone is insufficient.
...
The shared `choice-card-input` treatment owns visually hidden full-card controls, contained positioning, and label-level focus. `tests/builder-choice-presentation-contract.test.mjs` is the test-owned family and presentation registry: every guided family must retain explicit evidence, obsolete marker tokens stay absent, full-card controls retain native semantics, Decades content cards retain independent pressed states and the final-selection guard, forced-colours support remains present, and selected-state rails or cardinality/palette regressions cannot be introduced accidentally. `SemanticSortChoices` derives `data-selection-mode="multiple|single"` from its existing selection API; thin adapters expose the same hook on their controls. `data-selection-semantics="include|exclude"` explicitly overrides the ordinary cardinality palette. Shared exclusion selectors must also outrank control border shorthands so every excluded edge remains dashed. Selection-specific CSS tokens are separate from success colours.

## Guided stage intros (#230)

All nine guided families use presentation-only `CreationStageIntro`: `step`, optional `phase`, `title`, optional `description`, `headingId`, `headingRef`, and `tabIndex`. It renders one lightweight Step/phase kicker, h3 and optional helper before ordinary trays, summaries and controls. It owns no navigation, business rules or total stage count. `CreationHeader` remains visually unchanged; People retains its existing outer-header implementation and lifecycle.

Decades and People retain three stages; Franchises and TMDB Lists retain two; Studios and Networks retain Select/Configure/Appearance; Genres and Discover retain four. Streaming keeps Regions and Providers as two Select subviews. Append-only routes can finish before Appearance; selected-Folder Add Source keeps separate introductory presentation where its workflow differs. Results headings and useful nested sections remain.

## Draft creation

The family implementation descriptions below retain their original milestone scope. Native variants (#200), standalone Discover (#202), Shared Advanced (#218/#220), Preview-100 (#226) and shared guided presentation (#230) supersede earlier single-sort, eight-mode, ten-poster and stage-copy restrictions; current behavior is owned by the focused contracts and [Product Plan current state](./BUILDER_PRODUCT_PLAN.md#current-state).
...
This section describes single-node deletion. Collection-scoped multi-Folder removal uses the separate atomic #232 operation described at the top of this document.
...
- Collection menus in Edit / Sort folders / Delete folders / Delete collection order, Folder Edit/Delete menus, Edit source/Delete for supported source families, and Delete-only menus for unsupported sources;

...
- A coloured leading rail is never a selection indicator. Rails remain available only for non-selectable semantic messaging such as warnings, errors, or explanatory notices.
...
Issue #113 routes every existing `New collection` and `New folder` entry point through one contextual **What would you like to create?** dialog. The ordered registry now exposes **Blank**, **Decades**, **People**, **Franchises**, **TMDB Lists**, **Studios**, **Networks**, **Genres**, then **Streaming**, with explicit per-scope support and fail-closed unknown-family handling. Blank immediately delegates to the exact existing draft helper and therefore preserves the defaults, naming, desktop selection, mobile level, and card-scrolling behavior above without adding a naming or review step.

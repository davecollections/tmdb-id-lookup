...
## Optional Source names in selected-folder Add (#257)

Generated names remain the default across Decades, Movie franchise, Genres, Networks, People, Streaming, Studios, TMDB Lists and Discover. The final Add surface has an initially collapsed **Source name** / **Source names** disclosure after candidates are known; rows mount only when opened. It shows **Generated automatically.** or **N customised.** Ordinary families expose one final title per current physical candidate in existing Save order, with immutable entity/media/role/region/sort context. Review shows the effective saved title. There is no naming stage and no unique-title requirement.

Customisation changes only `editable.title`. The existing candidate, duplicate and Preview identity contracts exclude that display title. Canonical entity names, IDs, provider, media, role, sort, filters, region and ordering remain unchanged. Untouched output is identical. UI overrides never enter exported JSON. Preview uses the generated candidate and its unchanged request payload.

Overrides use existing physical keys: Franchise `movieFranchiseDuplicateIdentity`; TMDB List `tmdbListPhysicalIdentity`; People/Studio/Network their native variant keys; Genre/Streaming/Decade the existing `sourcePreviewVariantKey` DISCOVER recipe identity. A surviving exact key retains its custom title; removed keys may return within this dialog but never count or save while dormant. A changed recipe gets its generated name, even when it occupies the same row. Streaming therefore no longer carries a custom title to a different sole sort in selected-folder Add.

Blank drafts remain editable and return to automatic on blur, collapse or Add. A generated-equivalent trimmed value is automatic, without fuzzy/case folding. **Reset name** restores only the current generated title; **Reset all** affects current rows only. Nonblank invalid titles retain a labelled field and linked plain-language error and block Add. The existing Source title rules apply, with no new length limit. Generated-title validation is relaxed only for the selected-folder Add save path; structural and recipe validation remains strict.

Discover retains one **Base name**, using its existing generator and media/sort suffixes; the customised count is one base-name override. It has no per-output inputs. Genre sibling-folder routing and all guided Collection/Folder naming remain unchanged, as does Source Edit. This resolves D02 only for selected-folder Add; see the focused tests in [Testing](../TESTING.md#optional-source-naming-257).
...
2. **Review** contains Back and Close, the official title, a canonical external link from the validated positive numeric TMDB collection ID to `https://www.themoviedb.org/collection/<id>`, movie count, the optional **Source name** disclosure, the fixed recipe, a poster or placeholder, and the explicit poster-only **Preview titles** action. The Source name uses the canonical collection name until customised, retains the serialized `title` field, and returns a blank draft to automatic naming. The link opens in a new tab with `noopener noreferrer`, a visible external indicator, meaningful accessible naming, and wrapping that preserves long titles at mobile widths. Unsupported or invalid entity data renders no link. Review has exactly one footer action: Add source, Add anyway after a duplicate warning, or a disabled adding state. The earlier expandable contained-title text list is absent; the non-interactive count remains, and title inspection uses the shared nested poster Preview.

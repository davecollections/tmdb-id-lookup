...
Advanced retains minimum/maximum rating, minimum votes, original language, origin country and contextual Genre exclusions. It also supports `withKeywords`, `withoutKeywords`, `withCompanies`, `withoutCompanies`, TV-only `withNetworks`, `watchRegion`, `withWatchProviders` and `withoutWatchProviders`. All four entry points use one collapsed Advanced disclosure. The shared Discover controls appear in this order: Minimum votes, Minimum rating, Maximum rating; Original language and Origin country; Genres; Keywords; applicable dates/year; Studios and TV Networks; then Streaming providers and Watch region. Thresholds share one compact desktop row and stack on phones. Include and selected included Genres are green, Exclude and selected excluded Genres are red, and Any/All remains cyan; structural Decades Genres retain separate concrete-source semantics. Optional fields compile independently for each Movie/TV candidate; Movies never receive `withNetworks`.

The selected period exclusively owns `releaseDateGte`/`releaseDateLte`. There are no editable date bounds or redundant `year`. A structural Genre remains one fixed official `withGenres`; conflicting exclusions are reported, never pruned.

Exclusion contexts use **Shared**, **Using default** and **Custom**. An absent per-Decade key inherits Shared; an own empty ordinary array `[]` or whole per-Genre map `{}` is intentionally blank Custom. Customise starts blank; Clear selections keeps Custom; Use default deletes the own key. Later Shared changes affect only inherited contexts, even if a Custom value happens to equal Shared. Per-Genre fallback remains whole-map, never per-leaf. Structural `genreNamesByDecade` retains its separate All selected bulk behaviour. Frozen plans preserve sparse key presence and blank Custom values through rebuild/application.

Physical Decade Edit reuses the exclusion/help secondary surfaces, focus restoration and inert underlay. Touched fields and equivalent mirrors alone are patched; unsafe optional groups remain preservation-only. Period plus Genre stays Decades, while overlapping single-provider/region recipes route to Full Discover.
...
## Curated Folder artwork (#224)

Decades creation uses a Builder-only static projection of the exact canonical URLs in the [published Decades manifest](https://raw.githubusercontent.com/davecollections/nuvio-assets/main/assets/collection_covers/decades/manifest.json), approved release `26e65a4f04af83ff4067e69680a86cc0783d023e`. No runtime manifest request or refresh service is added. The resolver is `builder/src/source-add/decades-folder-artwork.js`; source generation is unchanged.

Every effective child Source must pass the existing strict canonical Decades classifier and belong to one catalogue Decade. Overview, individual years, Before 1950, structural Genres, multiple sorts and valid #220 Advanced filters can share that identity. The Builder ID `1950s-and-earlier` maps explicitly to artwork key `1950s-earlier`, still covering everything through 1959. Names, editor routing and transient plan metadata do not establish identity. Empty, multi-Decade, unrelated, opaque/addon or unsupported effective Source sets receive no Decades suggestion; the classifier is not broadened for artwork.

Actual physical media determines `movies`, `series` or `mixed`, including single-media Genre-only folders in nominally Both hierarchies. Empty counterparts get no artwork; their existing controller application safeguards remain unchanged. Poster uses `poster`/`posterFocus`, Square uses `square`/`squareFocus`, and Landscape uses `landscape`/`focus`. Each pair shares its identity's `hero` and `titleLogo`. All assets are WebP; variants never borrow artwork from one another.

The existing hierarchy planner assigns the four artwork fields while preserving the Poster default, chosen title visibility and disabled Focus default. Ordinary Add Source never replaces Folder artwork. Export/reopen and renames retain source-derived assistance through the common Folder Edit authority, without persisted provenance or serializer changes. Only explicit shape changes replace exact recognized same-identity curated cover/focus values, independently; custom/imported/wrong-identity values and hero/logo stay exact. Blank fields retain opt-in assistance. Focus URLs never toggle `focusGifEnabled`; unknown imported shapes remain unchanged until explicitly replaced.

Parent covers are excluded. Collection `backdropImageUrl` has fallback Folder semantics in Modern View and is not a parent collection-card cover. No collection artwork, contents, order, filter or Source-generation behavior changes.

Validation: `tests/decades-artwork.test.mjs` covers all 24 source identities and 72 shape selections against the approved static projection. For representative live-artwork UI journeys, set `TMDB_DECADES_ARTWORK_ONLY=1` and run `node --test --test-name-pattern="mounted Decades artwork" tests/builder-source-edit-mounted.test.mjs`. This uses the published manifest and real canonical images for Movies, Series and Mixed creation, renamed export/reopen, three shape transitions and preservation, plus the five required phone widths, desktop and short height. The ordinary broad suite retains the three representative journeys. Screenshots use the existing `TMDB_204_SCREENSHOTS` directory setting outside Git.
...
Square changes generic folder presentation only. Decades curated-artwork integration remains deferred; no Square asset URL is manufactured.

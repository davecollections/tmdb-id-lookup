...
- The manual package contains one deterministic combined fixture that is the exact ordered concatenation of four retained component collections: 29 sources total, 19 essential. Use the combined file once because NuvioTV upserts collections by ID while NuvioMobile replaces its current list on each import. Component files are targeted reruns only; Mobile still requires the combined fixture for cross-component comparisons such as essential U1→M1, and unknown-filter export/sync evidence is captured with the combined fixture installed.

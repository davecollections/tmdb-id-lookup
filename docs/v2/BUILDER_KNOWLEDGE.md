...
**Merged through issue [#73](https://github.com/davecollections/tmdb-id-lookup/issues/73) / PR [#80](https://github.com/davecollections/tmdb-id-lookup/pull/80), with deterministic validation, owner runbook review, and the hosted producer/consumer recovery drill complete, 2026-08-06:** positive-consumption `collect`-mode Company Movie, Company Series, Network Series, Company repair, and Network repair outputs have a commit-only recovery contract before September activation. The full operator and security contract is documented in [`ENTITY_COUNT_RECOVERY.md`](./ENTITY_COUNT_RECOVERY.md).
...
- Deterministic synthetic drills cover exact restore, repeat no-op, corrupt payload/manifest, wrong identities, unsafe paths and inventories, remote advance/rejection, same-file hybrid rejection, complete newer legacy preservation, mixed legacy failure, positive-usage reconciliation, and zero-consumption exclusion. The post-merge hosted fixture producer and automatic consumer passed the real artifact, exact recovery, repeat no-op, Linux symlink, corruption/traversal, hybrid-output, safe remote-advance, cohort, and zero-request gates without production-state writes. Artifact `8954448669` is retained through `2026-11-04T03:11:53Z`; September recovery readiness is complete and issue #73 is closed. Issue #73 Actions references remain pinned to reviewed full SHAs.
- Schedules, September 2026 activation, request allowances, partitions, production schemas, stable v1, Builder source, Worker, genre flow, dependencies, lockfiles, production data, and `nuvio-assets` remain unchanged. Existing monthly refreshes continue; any later activation decision remains separately reviewed. The bounded Company Movie/Series live smoke under issue #84 has not run, and the read-only Network comparison remains separate future work.
...
11. Advanced Discover creation remains the next recorded Builder feature milestone.
...
| 2026-08-06 | Durable typed-count recovery merged; the hosted producer/consumer drill passed exact recovery, repeat no-op, Linux symlink, corruption/traversal, hybrid-output, remote-advance, cohort and zero-request gates without production writes; retained artifact expiry is 2026-11-04; September readiness is complete | [TMDB ID Lookup issue #73](https://github.com/davecollections/tmdb-id-lookup/issues/73), [PR #80](https://github.com/davecollections/tmdb-id-lookup/pull/80), and [`ENTITY_COUNT_RECOVERY.md`](./ENTITY_COUNT_RECOVERY.md) |
...
- **2026-08-06 — Durable typed writer recovery completion:** retain the established monthly collectors, reservations, usage, progress, maintenance commit action, publication, schedules, activation, allowances, partitions, and Company/Network count semantics; keep the merged API-bound, hash-verified, no-rebase recovery contract; accept the post-merge hosted producer/consumer drill as passing with zero TMDB/network requests and no production writes; retain its artifact through 2026-11-04; mark September recovery readiness and issue #73 complete; and keep the unrun issue #84 Company Movie/Series smoke plus read-only Network comparison separate.

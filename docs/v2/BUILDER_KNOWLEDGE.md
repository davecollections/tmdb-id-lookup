...
- The ordinary Company days 1–14 and Network days 1–2 catalogue refreshes remain. They still refresh entity details and the legacy compact `t` / CSV `titles_count` values used by V1.
...
- Cached Company and Network catalogues remain the V2 Search/Add discovery source. After an exact entity is selected, a future V2 Company or Network flow should fetch fresh media-specific counts on demand. Counts are informational and non-blocking; unavailable counts must not prevent source creation.
...
10. Native physical-source editing — integrated through issue #78 / PR #79 for Movie Collection and People through a registry/adapters seam. Focused checks, bounded local desktop/mobile browser QA, the sanitized fixture package, and owner review are complete. The full current-client V2 edit/export round trip is deliberately deferred until V2 exposes export and is not an unfinished integration gate.
11. Company source creation is the next recorded Builder feature milestone, using cached catalogues for discovery and fresh informational/non-blocking counts only after selection.
12. Network source creation follows the same on-demand count boundary.
13. Advanced Discover creation follows Company and Network.
14. Broader deliberate V2 UI/UX and artwork-runtime work follows at the appropriate source stages.

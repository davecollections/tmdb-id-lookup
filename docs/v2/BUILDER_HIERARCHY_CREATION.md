...
This document owns the rules shared by guided **New Collection** and **New Folder** hierarchy families. Family documents such as [`BUILDER_DECADES.md`](./BUILDER_DECADES.md), [`BUILDER_PEOPLE.md`](./BUILDER_PEOPLE.md), [`BUILDER_FRANCHISES.md`](./BUILDER_FRANCHISES.md), [`BUILDER_STUDIOS.md`](./BUILDER_STUDIOS.md), [`BUILDER_NETWORKS.md`](./BUILDER_NETWORKS.md), and [`BUILDER_GENRES.md`](./BUILDER_GENRES.md) continue to own family-specific source contracts, defaults, naming, artwork authority, duplicate strictness, and optional features. Repository implementation, deterministic tests, and confirmed owner evidence override obsolete plans.

The merged guided hierarchy families are **Decades, People, Franchises, Studios, and Networks**. **Genres** is implemented for both scopes through issue [#130](https://github.com/davecollections/tmdb-id-lookup/issues/130) on its dedicated branch and remains unstaged and uncommitted. Owner Worker version `857c1fa3-e62d-4fd8-9321-9573aedb1906` and the first required production Worker/TMDB/image-CDN Preview validation passed on 2026-08-21. Dave then confirmed deployment of the corrected 10,479-byte source requiring canonical `include_adult=false`; direct rejection checks and the complete mounted production Worker/TMDB/image-CDN validation passed. Final desktop/physical-phone owner review is complete; repository commit/PR/merge/publication remain open. Its merged issue #110 selected-folder Add Source behavior remains separate and unchanged.
...
| Concern | Decades | People | Franchises | Studios | Networks | Genres |
| --- | --- | --- | --- | --- | --- | --- |

...
Workspace Import has two layouts inside one continuously mounted modal. **Acquire** retains the method column beside selected content at 900px and wider, and stacks methods above content on narrower screens. Nuvio Connect/profile/PIN, the file picker and pasted JSON live here. Workspace-specific CSS removes Welcome's large content minimum height and stretched form rows; short panels use natural height, while longer content remains scrollable. The existing bounded scroll/focus helper reveals the acquisition heading without opening a keyboard.

**Review** begins when any method obtains its incoming snapshot. It hides the method layout and divider and gives `CollectionImportReview` the entire existing modal content width. The fixed header has **Back**, **Review import** and **Close**; quiet, programmatically associated context identifies Nuvio's avatar/name/profile index, **File · filename**, or **Pasted JSON**. Long context is bounded to two lines without shrinking navigation targets. Nuvio account email and progress remain in Acquire rather than repeating in Review. Counts, update time, local-snapshot context, warnings and collapsed **View import notes** precede shared decisions. Add/Merge/Replace and Merge artwork cards stay vertically stacked at every width. One body scroll owner sits between the fixed header and footer; final **Cancel / Import to Dingo** actions stay reachable outside it. Review does not show Disconnect.

Replace stays in that same full-width modal, with **Back / Replace current project? / Close**, the existing irreversible warning, and footer **Keep current work / Replace current project**. Initial focus goes to the safe footer action. Header Back and Keep current work both return to the retained Review and mode without mutation. Empty current projects also use full-width Review but omit unnecessary mode/policy choices.

`NuvioImportFlow` owns shared login, account, profiles, avatar/PIN, snapshot, expiry, notices and review state; it owns no portal, dialog, trap or body lock. `WorkspaceImportDialog` owns the sole backdrop, dialog semantics, Visual Viewport handling, scroll area, focus trap, body lock and Close/Cancel, and places shared review presentation and actions in the appropriate body/footer slots. Welcome and standalone Nuvio keep their existing presentation.

The workspace Nuvio card is an ordinary selected method (`aria-pressed`), with no dialog-trigger semantics. Review header Back returns to the exact acquisition method: Nuvio's selected profile and valid memory-only session, the retained native file input, or the retained pasted text. It discards reviewed snapshot authority; reviewing again resets mode/artwork choices. Method changes are available only after Back and preserve drafts/session. The flow remains mounted but inactive while a local method is selected, with no hidden-stage focus. Active login, refresh, PIN and pull operations disable switching through the existing synchronous busy guard. Outer Close cancels pending request/review authority and restores workspace Import; explicit Disconnect remains available in Nuvio Acquire. File/JSON retain local-browser privacy copy; Nuvio keeps the direct-to-Nuvio credential, unsaved-login, memory-only-session and no-mutation explanation.

...
Set `BUILDER_FIND_ONLY=1` and run `node --test --test-name-pattern="mounted Find" tests/builder-bulk-edit-mounted.test.mjs`. Find reuses the existing browser lifecycle/CI workspace group with `tests/helpers/project-find-mounted.mjs` and a focused local fixture. It covers 112 layouts at 360/384/393/402/412/899/900/901/1280, short 320px heights, desktop/phone 200% text, forced colours and reduced motion. Six native touch/pointer jumps supplement thirty-six keyboard jumps across all three node types, four Folder/Source jumps after keyboard-like viewport restoration, and four middle-item Folder/Source jumps on phone/desktop (50 total). They check exact parent selection, card focus, actual Find-specific centered scrolling (or end-of-scroll clamping with at least 64px lower-edge clearance), reduced motion and clearing the mobile override on ordinary selection. Additional cases cover empty projects, opening/search/cancel snapshot identity, duplicate jumps, controlled stale current-state substitution, busy/modal/reorder guards, native Tab/Escape/Enter, accessible names, the 300ms announcement delay, cyan input focus and zero Find requests. `BUILDER_FIND_SCREENSHOT_DIR` optionally writes owner screenshots and measurements outside Git.

Twenty-eight keyboard-geometry states at 360/393px keep the 852px layout viewport intact while independently shrinking/panning the Visual Viewport (down to 320px high with a 42px top offset), then restoring it. Empty/many-result cases assert full opaque portal coverage, one backdrop/body lock, full-screen inner sizing, visible header/search, results above the keyboard boundary, stable scrolling, no horizontal overflow and Close focus restoration while still reduced. Desktop assertions cover compact idle/empty height, natural result growth, the 640px cap and one result scroll owner. These are mounted browser simulations and screenshots, not physical iOS keyboard certification; owner iPhone Safari retesting remains necessary.

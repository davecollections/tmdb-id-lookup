...
import { loadGenreSuggestions, planCuratedFolderTileShapeTransition, planCuratedFolderFocusShapeTransition } from "../folder-artwork-suggestions.js";
export function advancedDiscoverArtwork(draft, tileShape = "POSTER") {
 const filters = draft?.filters ?? {};
 if (!["POSTER", "SQUARE", "LANDSCAPE"].includes(tileShape)) return null;
...
export function advancedDiscoverArtworkSuggestions(draft) {
 if (advancedDiscoverArtwork(draft) === null) return null;
 const genre = officialGenreReference(draft.mediaMode === "series" ? "TV" : "MOVIE", Number(draft.filters.withGenres));
 return loadGenreSuggestions({ authority: "genre", genreName: genre.name, key: `genre:${genre.name}` });
}

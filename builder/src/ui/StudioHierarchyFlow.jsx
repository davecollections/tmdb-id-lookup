...
function ConfigureStep({ studios, knownSeriesCounts, outcomes, mediaMode, sortOptionIds, onMediaChange, onSortChange, onPreview, onRemove, placement, options, onAdvancedChange }) {
	...
	return (
		<section className="studio-hierarchy-configure" aria-labelledby="studio-hierarchy-configure-title">
			...
			<StudioAdvancedOptions draft={options} onChange={onAdvancedChange} entities={studios} />
...
export function StudioHierarchyFlow({
	...
	const configureEntries = useMemo(() => {
		...
		return chosen.map((studio) => {
			const result = buildStudioSourceDrafts(studio, { choices: mode?.choices ?? [], sortOptionIds: options.sortOptionIds, filters: options.filters, genreOverrides: options.genreOverrides, titleMode: STUDIO_SOURCE_TITLE_MODES.HIERARCHY });
	...
	}, [chosen, destinationCollectionInternalId, options.mediaMode, options.sortOptionIds, options.filters, options.genreOverrides, project, scope]);

import { creationContext, destinationContext } from "./creation-context.js";
...
import { DiscoverFamilyAdvancedOptions, DiscoverFamilyAdvancedSummary } from "./DiscoverFamilyAdvancedOptions.jsx";
...
function SelectableProviderResult({ provider, regionCodes, checked, onToggle }) {
	...
	return (
		<label className={`add-source-result studio-result studio-result-selectable streaming-provider-result streaming-provider-selectable${checked ? " is-selected" : ""}`} data-streaming-provider={provider.id} data-selection-mode="multiple">
...
function ProviderSelectionStep({
	...
	return (
		<section className="streaming-hierarchy-select" aria-labelledby="streaming-hierarchy-services-title">
			<CreationStageIntro step={1} phase="Select" title="Choose Streaming services" headingId="streaming-hierarchy-services-title" headingRef={headingRef} tabIndex={-1} />
			...
			{singleRegion ? <div className="studio-search-controls streaming-provider-browse" aria-label="Provider browse mode"><button data-selection-mode="single" type="button" aria-pressed={browseMode === STREAMING_PROVIDER_BROWSE_MODES.TOP} onClick={() => onBrowseModeChange(STREAMING_PROVIDER_BROWSE_MODES.TOP)}>Top providers</button><button data-selection-mode="single" type="button" aria-pressed={browseMode === STREAMING_PROVIDER_BROWSE_MODES.ALL} onClick={() => onBrowseModeChange(STREAMING_PROVIDER_BROWSE_MODES.ALL)}>A–Z</button></div> : null}
...
function ConfigureStep({ advanced = { filters: {} }, onAdvancedChange, advancedErrors = [], providers, regions, mediaChoice, sortOptionIds, groupingMode, onSortChange, onGroupingChange, onPreview, onRemove, headingRef }) {
	return (
		<section className="studio-hierarchy-configure streaming-hierarchy-configure" aria-labelledby="streaming-hierarchy-configure-title">
			<CreationStageIntro step={2} phase="Configure" title="Configure Streaming services" headingId="streaming-hierarchy-configure-title" headingRef={headingRef} tabIndex={-1} />
			...
			<DiscoverFamilyAdvancedOptions value={advanced} onChange={onAdvancedChange} mediaMode={mediaChoice} fixedFilters={{ watchRegion: regions[0]?.code, withWatchProviders: providers.map((provider) => provider.id).join("|") }} fixedProviderContext={providers.map((provider) => provider.name).join(", ") + " · " + regions.map((region) => region.code).join(" · ") + " · services and regions stay fixed"} />
	...
	return (
		<details className="streaming-folder-names" data-streaming-folder-name-count={folders.length}>
			...
			<div className="streaming-folder-name-list">{folders.map((folder) => {
				...
				return <div className="editor-field streaming-folder-name-field" key={folder.key}><label htmlFor={inputId}>{folder.generatedTitle}</label><RequiredNameInput id={inputId} value={value} hidden={hiddenEverywhere} error={error?.message} describedBy={hiddenEverywhere ? "streaming-folder-titles-hidden-help" : undefined} onChange={(event) => onChange(folder.key, event.target.value)} />{custom ? <div><button type="button" onClick={() => onUseDefault(folder.key)}>Use default name</button></div> : null}</div>;
			})}</div>
...
function ReviewStep({ newCollectionFolderNames, planResult, intentConfiguration, newCollectionElsewhereEvidence, destinationCandidates, selectedDestination, onDestinationChange, options, originalScope, activeScope, destinationCollectionTitle, collectionDisplayContext, folderTitleDrafts, folderTitleErrors, onOptionsChange, onFolderTitleChange, onUseDefaultFolderTitle, diagnostic, headingRef }) {
	...
	const otherErrors = (planResult?.errors ?? []).filter((entry) => entry.path !== "$streamingHierarchy.collectionTitle");
	const existingScope = activeScope === "new-folder";
	const routedExisting = originalScope === "new-collection" && existingScope;
	const nameFolders = existingScope ? plan?.newFolders ?? [] : newCollectionFolderNames;
	...
	return (
		<section className="studio-hierarchy-review studio-hierarchy-appearance streaming-hierarchy-review" aria-labelledby="streaming-hierarchy-review-title">
			<CreationStageIntro step={3} phase={choiceRequired ? "Destination" : existingScope && plan?.counts.newFolderCount === 0 ? "Review" : "Appearance"} title={choiceRequired ? "Choose destination" : existingScope && plan?.counts.newFolderCount === 0 ? "Review" : "Appearance"} headingId="streaming-hierarchy-review-title" headingRef={headingRef} tabIndex={-1} />
			{summaryConfiguration?.advanced ? <DiscoverFamilyAdvancedSummary ui={options.advanced?.ui} value={summaryConfiguration.advanced} mediaMode={summaryConfiguration.mediaChoice} /> : null}
			...
			{choiceRequired ? <><NewCollectionElsewhereEvidence evidence={newCollectionElsewhereEvidence} choicePending collectionDisplayContext={collectionDisplayContext} />{diagnostic ? <div className="editor-diagnostics" role="alert"><p>{diagnostic.message}</p></div> : null}</> : <>
				{plan ? <><section className="streaming-change-summary" aria-labelledby="streaming-change-summary-title"><h4 id="streaming-change-summary-title">{existingScope ? "What will change" : "What will be created"}</h4><div className="decades-plan-totals streaming-plan-totals" data-plan-scope={activeScope} role="group" aria-label="Plan totals">{existingScope ? <><div><strong>{plan.counts.existingFolderAdditionCount}</strong><span>Existing folder{plan.counts.existingFolderAdditionCount === 1 ? "" : "s"} updated</span></div><div><strong>{plan.counts.newFolderCount}</strong><span>New folder{plan.counts.newFolderCount === 1 ? "" : "s"}</span></div><div><strong>{plan.counts.newSourceCount}</strong><span>Source{plan.counts.newSourceCount === 1 ? "" : "s"} to add</span></div></> : <><div><strong>{plan.counts.collectionCount}</strong><span>{plan.counts.collectionCount === 1 ? "Collection" : "Collections"}</span></div><div><strong>{plan.counts.folderCount}</strong><span>Folder{plan.counts.folderCount === 1 ? "" : "s"}</span></div><div><strong>{plan.counts.sourceCount}</strong><span>Source{plan.counts.sourceCount === 1 ? "" : "s"}</span></div></>}</div></section>
				...
				{activeScope === "new-collection" ? <><div className="editor-field"><label htmlFor="streaming-collection-name">Collection name</label><RequiredNameInput id="streaming-collection-name" value={options.collectionTitle} hidden={options.hideCollectionTitle} describedBy={options.hideCollectionTitle ? "streaming-collection-title-hidden-help" : undefined} error={requiredNameMessage(planResult?.errors, "$streamingHierarchy.collectionTitle", options.collectionTitle)} onChange={(event) => onOptionsChange({ collectionTitle: event.target.value })} /><HiddenTitleFieldHelp id="streaming-collection-title-hidden-help" hidden={options.hideCollectionTitle} kind="collection" /></div><FolderNameEditor folders={nameFolders} drafts={folderTitleDrafts} errors={folderTitleErrors} hiddenEverywhere={options.folderTitleVisibility === "HIDE_EVERYWHERE"} onChange={onFolderTitleChange} onUseDefault={onUseDefaultFolderTitle} /><TitleOptions idPrefix="streaming-hierarchy" collectionTitleVisibility={{ checked: options.hideCollectionTitle, onChange: (hideCollectionTitle) => onOptionsChange({ hideCollectionTitle }), descriptionId: "streaming-hide-title-help", controlName: "streamingHideNuvioTitle" }} folderTitleVisibility={{ selectedId: options.folderTitleVisibility, name: "streaming-folder-title-visibility", onChange: (folderTitleVisibility) => onOptionsChange({ folderTitleVisibility }) }} /><fieldset className="editor-field editor-choice-field"><legend>Collection layout</legend><HierarchyCollectionPresentationControls selectedId={options.viewMode} name="streaming-collection-layout" showAllTab={options.showAllTab} onPresentationChange={onOptionsChange} showAllDescriptionId="streaming-all-tab-help" showAllControlName="streamingShowAllTab" /></fieldset><PresentationSwitch label="Pin collection to top" description="Keeps this collection near the top of Nuvio." descriptionId="streaming-pin-help" controlName="streamingPinToTop" checked={options.pinToTop} onChange={(pinToTop) => onOptionsChange({ pinToTop })} /></> : plan ? <><div className="franchise-inherited-summary"><strong>Collection settings stay unchanged.</strong><span>{activeDestinationLabel} · {plan.destination.viewMode === "ROWS" ? "Rows" : "Tabs"}. This operation does not rename or reconfigure the existing collection; appearance choices below apply only to new folders.</span></div><FolderNameEditor folders={nameFolders} drafts={folderTitleDrafts} errors={folderTitleErrors} hiddenEverywhere={options.folderTitleVisibility === "HIDE_EVERYWHERE"} onChange={onFolderTitleChange} onUseDefault={onUseDefaultFolderTitle} />{nameFolders.length ? <TitleOptions idPrefix="streaming-hierarchy" folderTitleVisibility={{ selectedId: options.folderTitleVisibility, name: "streaming-folder-title-visibility", onChange: (folderTitleVisibility) => onOptionsChange({ folderTitleVisibility }) }} /> : null}</> : null}
				{!plan && (otherErrors.length > 0 || !planResult?.errors?.length) ? <div className="editor-diagnostics" role="alert"><p>{otherErrors[0]?.message ?? "The Streaming plan could not be prepared."}</p></div> : null}
...
export function StreamingHierarchyFlow({
	...
	const hierarchyIntentOptions = useMemo(() => Object.freeze({
		...
		providers: chosen,
	}), [chosen, options.folderTitleVisibility, options.groupingMode, options.mediaChoice, options.sortOptionIds, options.advanced, projectRevision, selectedRegions, validFolderTitleOverrides]);
	...
	const activeNewFolderKeys = new Set(activeScope === "new-collection" ? destinationResult?.logicalFolderKeys ?? [] : planResult?.ok ? planResult.plan.newFolders.map((folder) => folder.key) : []);
	...
	const folderTitleErrors = useMemo(() => options.folderTitleVisibility === "HIDE_EVERYWHERE" ? new Map() : new Map(Object.entries(folderTitleDrafts)
		...
		.map(([key]) => [key, { message: "Enter a folder name." }])), [activeNewFolderKeySignature, folderTitleDrafts, options.folderTitleVisibility]);
	...
	async function submit(event) {
		...
		if (step === "configure") {
			if (!chosen.length || !options.sortOptionIds.length || (!(scope === "new-collection" ? newCollectionPlanResult : fixedExistingPlanResult)?.ok && !onlyRequiredNameErrors(scope === "new-collection" ? newCollectionPlanResult : fixedExistingPlanResult, ["$streamingHierarchy.collectionTitle"]))) return;
	...
	const primaryDisabled = catalogueState.status !== "success" || (activeStage === "regions" ? selectedRegions.length === 0 : activeStage === "providers" ? chosen.length === 0 : activeStage === "configure" ? chosen.length === 0 || options.sortOptionIds.length === 0 || (!(scope === "new-collection" ? newCollectionPlanResult : fixedExistingPlanResult)?.ok && !onlyRequiredNameErrors(scope === "new-collection" ? newCollectionPlanResult : fixedExistingPlanResult, ["$streamingHierarchy.collectionTitle"])) : destinationChoiceRequired || (!planResult?.ok && !onlyRequiredNameErrors(planResult, ["$streamingHierarchy.collectionTitle"])) || (planResult?.plan?.conflicts.length ?? 0) > 0 || (activeScope === "new-folder" && planResult?.plan?.counts.newSourceCount === 0 && !routedExistingNoChanges) || isApplying);
	...
	const primaryLabel = activeStage === "regions" ? "Continue to Services" : activeStage === "providers" ? "Continue to Configure" : activeStage === "configure" ? destinationChoiceRequired ? "Continue to Destination" : activeScope === "new-folder" && planResult?.plan?.counts.newFolderCount === 0 ? "Continue to Review" : "Continue to Appearance" : destinationChoiceRequired ? "Choose a destination" : routedExistingNoChanges ? "Close" : isApplying ? activeScope === "new-folder" ? "Applying…" : "Creating…" : activeScope === "new-collection" ? planResult?.plan?.elsewhereEvidence?.overlap === "complete" ? "Create duplicate collection" : guidedCreateActionLabel(activeScope, planResult?.plan?.counts) : changesOnlyNewFolders ? guidedCreateActionLabel(activeScope, planResult?.plan?.counts) : "Apply changes";
	const descriptions = { regions: "Choose one or more regions. Search stays inactive until you use it.", providers: "Choose eligible services in folder order.", configure: "Choose sources and folder grouping, then preview the configured sources.", review: destinationChoiceRequired ? "Choose where these Streaming sources should go." : "Review what will be created or updated before you finish." };
	const interactionOverlayOpen = Boolean(preview || duplicateConfirmation);

	return <>
		<CreationHeader title="Create with Streaming" context={selectedCandidate ? destinationContext(collectionDisplayContext?.[selectedCandidate.collectionInternalId]?.label ?? selectedCandidate.collectionTitle) : creationContext(scope, destinationCollectionTitle)} description={descriptions[activeStage]} onBack={goBack} backAction={activeStage === "regions" ? "back-to-creation-launcher" : `back-to-streaming-${activeStage}`} backDisabled={isApplying} inactive={interactionOverlayOpen} onClose={onCancel} />
		<form className="add-source-form studio-hierarchy-form streaming-hierarchy-form" data-streaming-hierarchy-stage={activeStage} onSubmitCapture={handleRequiredNameSubmit} onSubmit={submit} noValidate>

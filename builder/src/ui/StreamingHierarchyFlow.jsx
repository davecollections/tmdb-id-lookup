...
function ReviewStep({ newCollectionFolderNames, planResult, intentConfiguration, newCollectionElsewhereEvidence, destinationCandidates, selectedDestination, onDestinationChange, options, originalScope, activeScope, destinationCollectionTitle, collectionDisplayContext, folderTitleDrafts, folderTitleErrors, onOptionsChange, onFolderTitleChange, onUseDefaultFolderTitle, diagnostic, headingRef }) {
	...
	return (
		<section className="studio-hierarchy-review studio-hierarchy-appearance streaming-hierarchy-review" aria-labelledby="streaming-hierarchy-review-title">
			<CreationStageIntro step={3} phase="Review" title={choiceRequired ? "Choose destination" : <>Review &amp; Appearance</>} headingId="streaming-hierarchy-review-title" headingRef={headingRef} tabIndex={-1} />
			...
			{choiceRequired ? <><NewCollectionElsewhereEvidence evidence={newCollectionElsewhereEvidence} choicePending collectionDisplayContext={collectionDisplayContext} />{diagnostic ? <div className="editor-diagnostics" role="alert"><p>{diagnostic.message}</p></div> : null}</> : <>
				{plan ? <><section className="streaming-change-summary" aria-labelledby="streaming-change-summary-title"><h4 id="streaming-change-summary-title">{existingScope ? "What will change" : "What will be created"}</h4><div className="decades-plan-totals streaming-plan-totals" data-plan-scope={activeScope} aria-label="Plan totals">{existingScope ? <><div><strong>{plan.counts.existingFolderAdditionCount}</strong><span>Existing folder{plan.counts.existingFolderAdditionCount === 1 ? "" : "s"} updated</span></div><div><strong>{plan.counts.newFolderCount}</strong><span>New folder{plan.counts.newFolderCount === 1 ? "" : "s"}</span></div><div><strong>{plan.counts.newSourceCount}</strong><span>Source{plan.counts.newSourceCount === 1 ? "" : "s"} to add</span></div></> : <><div><strong>1</strong><span>Collection</span></div><div><strong>{plan.counts.folderCount}</strong><span>Folder{plan.counts.folderCount === 1 ? "" : "s"}</span></div><div><strong>{plan.counts.sourceCount}</strong><span>Source{plan.counts.sourceCount === 1 ? "" : "s"}</span></div></>}</div></section>

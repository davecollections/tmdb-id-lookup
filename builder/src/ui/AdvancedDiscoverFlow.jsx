...
export default function AdvancedDiscoverFlow({ scope = "add-source", project, projectRevision, collectionInternalId = null, folderInternalId = null, studioProvider, networkProvider, streamingProvider, onBack, onCancel, onApply, initialDraft = null, onSave = null, bodyLockManaged = false }) {
 ...
 const namesValid = (!hierarchy || appearance.folderTitleVisibility === "HIDE_EVERYWHERE" || folders.every((folder) => isValidVisibleNuvioTitle(folder.title))) && (scope !== "new-collection" || appearance.hideCollectionTitle || isValidVisibleNuvioTitle(options.collectionTitle));
 ...
 <CreationHeader title={editing ? "Edit Full Discover" : hierarchy ? "Create with Discover" : "Add Discover sources"} context={editing ? contextLabel + (destinationLabel ? " - " + destinationLabel : "") : hierarchy ? creationContext(scope, destinationCollection?.editable.title) : sourceDestinationContext(project, destinationFolder)} description={instructions[page]} onBack={back} backDisabled={busy} inactive={secondaryActive} onClose={onCancel}
 ...
 <form className="add-source-form" onSubmitCapture={handleRequiredNameSubmit} onSubmit={submit} noValidate inert={preview.preview ? true : undefined}>
 ...
 {page === "filters" ? <>
  ...
  {scope === "new-collection" ? <div className="editor-field"><label htmlFor="discover-collection-name">Collection name</label><RequiredNameInput id="discover-collection-name" value={collectionTitle} hidden={appearance.hideCollectionTitle} error={!isValidVisibleNuvioTitle(collectionTitle) ? "Enter a collection name." : null} describedBy={appearance.hideCollectionTitle ? "discover-collection-hidden" : undefined} onChange={(e) => setCollectionTitle(e.target.value)} /><HiddenTitleFieldHelp id="discover-collection-hidden" hidden={appearance.hideCollectionTitle} kind="collection" /></div> : <p className="editor-field-help">These folders use the existing collection layout.</p>}
  ...
  {folders.map((folder) => <div className="editor-field" key={folder.key}><label htmlFor={"discover-folder-name-" + folder.key}>{folders.length === 1 ? "Folder name" : folder.mediaType === "MOVIE" ? "Movies folder name" : "Series folder name"}</label><RequiredNameInput id={"discover-folder-name-" + folder.key} value={folderSettings[folder.key]?.title ?? folder.title} hidden={appearance.folderTitleVisibility === "HIDE_EVERYWHERE"} error={!isValidVisibleNuvioTitle(folder.title) ? "Enter a folder name." : null} describedBy={appearance.folderTitleVisibility === "HIDE_EVERYWHERE" ? "discover-folder-hidden" : undefined} onChange={(e) => changeFolder(folder.key, { title: e.target.value })} /></div>)}

...
function CreationModeIcon({ icon }) {
	let drawing = null;
	if (icon === "blank") {
		...
		<ul className="add-source-scroll creation-option-list">
			{options.map((option, index) => (
				<li key={option.id}>
					...
					>
						<CreationModeIcon icon={option.icon} />
						<span className="creation-option-copy"><strong>{option.label}</strong>{option.supportingText ? <small>{option.supportingText}</small> : null}</span>
					</button>
				</li>
			))}
		</ul>

...
/**
 * Validates the builder hierarchy needed by migration without interpreting
 * legacy catalog projections.
 *
...
export function validateMigrationProject(project) {
	...
	if (!isPlainObject(project) || project.nodeType !== NODE_TYPES.PROJECT) {
		return [diagnostic("INVALID_PROJECT_NODE", "$", "The migration input must be a builder project node.")];
	}
	...
	if (!validateChildArray(project.collections, "$", "collections", errors)) {
		return finishValidation(errors, internalIds);
	}
 ...
 */
export function validateMigrationJsonCompatibility(project) {
	try {
...
function finishValidation(errors, internalIds) {
